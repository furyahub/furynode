...
export async function ibc(props) {
  const furyChainProps = getChainProps({ chain: "furynet" });
  ...
  const {
    ...
    ...props,
    chain: props.type === "issuer" ? props.chain : "furynet",
...
`);
  ...
  const srcAddr = await getAddress({
    binary: type === "receiver" ? furyChainProps.binary : props.binary,
    name,
  });
  const dstAddr = await getAddress({
    binary: type === "receiver" ? props.binary : furyChainProps.binary,
    name,
  });
