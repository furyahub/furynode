...
const chainAProps = getChainProps({ chain: "furynet", network: "testnet-1" });
...
test("ibc transfer between two chains", async () => {
  ...
  const channel =
    await $`ibc-setup channels --chain furynet --home ${chainBHome}/relayer/ | grep ${connection} | cut -d ' ' -f 1`;
