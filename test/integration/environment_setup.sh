required="${BASEDIR:?"Must be set to where you check out furynode"}"

cd $BASEDIR/test/integration

required="${DEPLOYMENT_NAME:?"Must be set to a deployment name like sandpit"}"
required="${FURY_SOURCE:?"Must be set to a fury address that contains fury, and the key must be in your keyring"}"
...
required="${FURYNODE:?"Must be set to furynode endpoint."}"
...
cp $BASEDIR/smart-contracts/build/contracts/FurynetTestToken.json $SOLIDITY_JSON_PATH
...
echo yarn -s --cwd /home/james/workspace/furynode/smart-contracts integrationtest:mintTestnetTokens  \
  --symbol $BRIDGE_TOKEN_ADDRESS \
...
echo; echo == lock eth
echo yarn -s --cwd $BASEDIR/smart-contracts integrationtest:sendLockTx --furynet_address $FURY_SOURCE \
  --symbol eth \
  --ethereum_private_key_env_var "ETHEREUM_PRIVATE_KEY" \
  --json_path $BASEDIR/smart-contracts/deployments/$DEPLOYMENT_NAME \
  --gas estimate \
...
echo yarn -s --cwd $BASEDIR/smart-contracts integrationtest:sendBurnTx \
  ...
  --bridgebank_address $BRIDGE_BANK_ADDRESS \
  --ethereum_address $ETHEREUM_ADDRESS \
  --furynet_address $FURY_SOURCE \
  --amount 17

echo; echo == burn efury from operator account
echo yarn -s --cwd /home/james/workspace/furynode/smart-contracts integrationtest:sendBurnTx \
  --symbol $BRIDGE_TOKEN_ADDRESS \
  --ethereum_private_key_env_var "OPERATOR_PRIVATE_KEY" \
  --json_path $BASEDIR/smart-contracts/deployments/$DEPLOYMENT_NAME \
  --gas estimate \
...
echo; echo == furynet transaction
echo $furynodecmd q tx --node $FURYNODE --chain-id $DEPLOYMENT_NAME 193EFB4A5D20BEC58ADE8BACEB38264870ADD8BAFEA9D6DAABE554B0ACBC0C93

echo; echo == all account balances
echo "$furynodecmd keys list --keyring-backend test --output json | jq -r '.[].address' | parallel $furynodecmd q auth account --node $FURYNODE -o json {} | grep coins"

echo; echo == burn ceth
echo $furynodecmd tx ethbridge burn \
...
echo; echo == send ceth
echo $furynodecmd tx send $FURY_SOURCE furysomedestination 100fury \
  --node $FURYNODE \
  --keyring-backend test \
  --fees 100000fury \
  --chain-id=$DEPLOYMENT_NAME  \
...
echo TOKENS=ceth,fury NTRANSFERS=2 python3 -m pytest -olog_level=DEBUG -olog_file=vagrant/data/pytest.log -v src/py/test_bulk_transfers_to_ethereum.py::test_bulk_transfers_from_furynet
