...
#
# Runs ebrelayer.  Normally, this is run by furynet_start_ebrelayer.sh;
# that file sets up the logs and runs furynet_run_ebrelayer in the background.
...
USER1ADDR=nothing python3 $TEST_INTEGRATION_PY_DIR/wait_for_fury_account.py $NETDEF_JSON $VALIDATOR1_ADDR
sleep 10
...
yes | furynoded keys delete $MONIKER --keyring-backend test || true
echo $MNEMONIC | furynoded keys add $MONIKER --keyring-backend test --recover

set_persistant_env_var EBRELAYER_DB "${TEST_INTEGRATION_DIR}/furynetrelayerdb" $envexportfile
