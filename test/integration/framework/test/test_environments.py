...
def get_validators(env):
    ...
    return {v["description"]["moniker"]: v for v in furynd.query_staking_validators()}


def test_transfer(env):
    furynd = env.furynd
    furynd.send_and_check(env.faucet, furynd.create_addr(), {furynet.FURY: 10 ** furynet.FURY_DECIMALS})
...
class TestFuryndEnvironment:
    ...
    def test_environment_setup_basic(self):
        ...
        env.add_validator()
        env.start()
        assert_validators_working(env, set("furynd-{}".format(i) for i in range(1)))
    ...
    def test_add_validator_before_and_after_start(self):
        ...
        env.start()
        env.add_validator()
        assert_validators_working(env, set("furynd-{}".format(i) for i in range(3)))
    ...
    def test_environment_fails_to_start_if_commission_rate_is_over_max(self):
        ...
        # What we get here is a "timeout waiting for furynd to come up".
        assert type(exception) == furynet.FuryndException
    ...
    def test_can_have_validators_with_same_moniker(self):
        ...
        try:
            ...
            assert len(furynd.query_staking_validators()) == 2
            env.add_validator(moniker="juniper", home=home2)
            assert len(furynd.query_staking_validators()) == 3
        finally:
    ...
    def test_balances(self):
        ...
        try:
            ...
            extra_accounts = {furynd.create_addr(): {"bar{}".format(j): (i * number_of_denoms + j + 1) * 10**25
            ...
            for i in range(number_of_validators):
                furynd = env._furynd_for(env.node_info[i])
                assert cosmos.balance_equal(furynd.get_balance(env.faucet), faucet_balance)
            ...
            for i in range(number_of_validators):
                ...
                for addr, balance in extra_accounts.items():
                    assert cosmos.balance_equal(furynd.get_balance(addr), balance)

            # Check funding
            for i in range(number_of_validators):
                addr = furynd.create_addr()
                amount = {"foo0": 100 * 10**18, "foo1": 100 * 10**18}
                env.fund(addr, amount)
                assert cosmos.balance_equal(furynd.get_balance(addr), amount)
            ...
            for i in range(number_of_validators):
                furynd_i = env._furynd_for(env.node_info[i])
                test_addr = furynd_i.create_addr()
                # Sending also requires fury, this way we're making sure that each validator has some
                admin_addr = env.node_info[i]["admin_addr"]
                furynd_i.send(admin_addr, test_addr, test_transfer_amount)
                for j in range(number_of_validators):
                    furynd_j = env._furynd_for(env.node_info[j])
                    furynd_j.wait_for_balance_change(test_addr, {}, test_transfer_amount)
        finally:
