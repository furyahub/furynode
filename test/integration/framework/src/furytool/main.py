...
from furytool import test_utils, run_env, cosmos, diagnostics, furynet, frontend, test_utils2
from furytool.run_env import Integrator, UIStackEnvironment, Peggy2Environment, IBCEnvironment, IntegrationTestsEnvironment
...
def main(argv):
    ...
    if what == "venv":
        ...
        import furytool.geth, furytool.eth
        geth = furytool.geth.Geth(cmd)
        datadir = os.path.join(os.environ["HOME"], ".furytool-geth")
        datadir = None
        signer_addr, signer_private_key = furytool.eth.web3_create_account()
