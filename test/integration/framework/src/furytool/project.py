import os
import json
from furytool.command import Command, ExecResult
...
class Project:
    ...
    def __init__(self, cmd: Command, base_dir: str):
        ...
        self.furytool_dir = project_dir("test", "integration", "framework")
    ...
    def __rm_files(self, level):
        ...
        if level >= 2:
            ...
            self.__rm_run_env_files()
            self.__rm(self.cmd.get_user_home(".furynode-integration"))
    ...
    def get_peruser_config_dir(self):
        return self.cmd.get_user_home(".config", "furytool")
    ...
    def __rm_run_env_files(self):
        ...
        self.__rm(self.project_dir("test", "integration", "furynetrelayerdb"))  # Probably obsolete on peggy2 TODO move to /tmp
    ...
    def clean(self):
        ...
        self.cmd.rmf(os.path.join(self.furytool_dir, "build"))
        if on_peggy2_branch:
    ...
    def npm_install(self, path: str, disable_cache: bool = False):
        ...
        if self.cmd.exists(node_modules):
            cache_tag_file = os.path.join(node_modules, ".furytool-cache-tag")
    ...
    def _ensure_build_dirs(self):
        for d in ["build", "build/repos", "build/generated"]:
            self.cmd.mkdir(os.path.join(self.furytool_dir, d))
    ...
    def generate_python_protobuf_stubs(self):
        ...
        generated_dir = os.path.join(self.furytool_dir, "build/generated")
        repos_dir = os.path.join(self.furytool_dir, "build/repos")
        ...
        proto_files = [
            os.path.join(project_proto_dir, "furynode/ethbridge/v1/tx.proto"),
            os.path.join(project_proto_dir, "furynode/ethbridge/v1/query.proto"),
            os.path.join(project_proto_dir, "furynode/ethbridge/v1/types.proto"),
            os.path.join(project_proto_dir, "furynode/oracle/v1/network_descriptor.proto"),
            os.path.join(project_proto_dir, "furynode/oracle/v1/types.proto"),
