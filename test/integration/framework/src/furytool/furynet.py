...
from furytool import command, cosmos, eth
...
# Furynet public network endpoints
BETANET = {"node": "https://rpc.furynet.finance", "chain_id": "furynet-1"}
TESTNET = {"node": "https://rpc-testnet.furynet.finance", "chain_id": "furynet-testnet-1"}
DEVNET = {"node": "https://rpc-devnet.furynet.finance", "chain_id": "furynet-devnet-1"}
...
def furynet_denom_hash(network_descriptor: int, token_contract_address: eth.Address) -> str:
    ...
    denom = f"furyBridge{network_descriptor:04d}{token_contract_address.lower()}"
    return denom

def furynet_denom_hash_to_token_contract_address(token_hash: str) -> Tuple[int, eth.Address]:
    m = re.match("^furyBridge(\\d{4})0x([0-9a-fA-F]{40})$", token_hash)
    if not m:
        raise Exception("Invalid furynet denom '{}'".format(token_hash))
...
def is_cosmos_native_denom(denom: str) -> bool:
    ...
    if on_peggy2_branch:
        return not str.startswith(denom, "furyBridge")
    else:
...
class Furynoded:
    ...
    def create_tokenregistry_entry(self, symbol: str, furynet_symbol: str, decimals: int,
        ...
        return {
            ...
            "denom": furynet_symbol,
            "base_denom": furynet_symbol,
            "path": "",
            "ibc_channel_id": "",
    ...
    # from_fury_addr has to be the address which was used at genesis time for "set-genesis-whitelister-admin".
    ...
    def token_registry_register(self, entry: TokenRegistryParams, from_fury_addr: cosmos.Address,
        ...
        assert self.keys_show(from_fury_addr)
        ...
        with self._with_temp_json_file(token_data) as tmp_registry_json:
            args = ["tx", "tokenregistry", "register", tmp_registry_json, "--from", from_fury_addr, "--output",
    ...
    def token_registry_register_batch(self, from_fury_addr: cosmos.Address, entries: Iterable[TokenRegistryParams]):
        account_number, account_sequence = self.get_acct_seq(from_fury_addr)
        token_registry_entries_before = set(e["denom"] for e in self.query_tokenregistry_entries())
        for entry in entries:
            res = self.token_registry_register(entry, from_fury_addr, account_seq=(account_number, account_sequence))
            check_raw_log(res)
            account_sequence += 1
    ...
    def build_start_cmd(self, tcp_url: Optional[str] = None, p2p_laddr: Optional[str] = None,
        ...
        args = [self.binary, "start"] + \
            (["--trace"] if trace else []) + \
            (["--minimum-gas-prices", fury_format_amount(*minimum_gas_prices)] if minimum_gas_prices is not None else []) + \
    ...
    def send(self, from_fury_addr: cosmos.Address, to_fury_addr: cosmos.Address, amounts: cosmos.Balance,
        account_seq: Optional[Tuple[int, int]] = None, broadcast_mode: Optional[str] = None
    ) -> JsonDict:
        ...
        args = ["tx", "bank", "send", from_fury_addr, to_fury_addr, amounts_string, "--output", "json"] + \
    ...
    def send_and_check(self, from_addr: cosmos.Address, to_addr: cosmos.Address, amounts: cosmos.Balance
        ...
        expected_tx_fee = {FURY: fury_tx_fee_in_fury}
    ...
    def get_balance(self, fury_addr: cosmos.Address, height: Optional[int] = None,
        disable_log: bool = False, retries_on_error: Optional[int] = None, delay_on_error: int = 3
    ) -> cosmos.Balance:
        base_args = ["query", "bank", "balances", fury_addr]
    ...
    def wait_for_balance_change(self, fury_addr: cosmos.Address, old_balance: cosmos.Balance,
        ...
        log.debug("Waiting for balance to change for account {}...".format(fury_addr))
        ...
        while True:
            new_balance = self.get_balance(fury_addr, disable_log=disable_log)
    ...
    # See https://github.com/Furynet/furynode/blob/master/docs/tutorials/clp%20tutorial.md
    ...
    def sign_transaction(self, tx: JsonDict, from_fury_addr: cosmos.Address, sequence: int = None,
    ...
    def wait_for_last_transaction_to_be_mined(self, count: int = 1, disable_log: bool = True, timeout: int = 90):
        log.debug("Waiting for last furynode transaction to be mined...")
        start_time = time.time()
        initial_block = self.get_current_block()
        while self.get_current_block() < initial_block + count:
            time.sleep(1)
            if time.time() - start_time > timeout:
                raise Exception("Timeout expired while waiting for last furynode transaction to be mined")
...
# Refactoring in progress - this class is supposed to become the successor of class Furynode.
...
class FurynodeClient:
    ...
    def send_from_furynet_to_ethereum(self, from_fury_addr: cosmos.Address, to_eth_addr: str, amount: int, denom: str,
        ...
        """ Sends ETH from Furynet to Ethereum (burn) """
        assert self.ctx.eth
        eth = self.ctx.eth
        ...
        if on_peggy2_branch:
            ...
            args = ["tx", "ethbridge", direction, to_eth_addr, str(amount), denom, str(cross_chain_ceth_fee),
                    ...
                    "--from", from_fury_addr,  # Mandatory, either name from keyring or address
                    "--output", "json",
                ] + \
                (["--generate-only"] if generate_only else []) + \
                self.furynode._fees_args() + \
                self.furynode._home_args() + \
                (self.furynode._keyring_backend_args() if not generate_only else []) + \
            ...
            args = ["tx", "ethbridge", direction] + \
                self.furynode._node_args() + \
                [from_fury_addr, to_eth_addr, str(amount), denom, cross_chain_ceth_fee] + \
                (self.furynode._keyring_backend_args() if not generate_only else []) + \
                self.furynode._fees_args() + \
    ...
    def send_from_furynet_to_ethereum_grpc(self, from_fury_addr: cosmos.Address, to_eth_addr: str, amount: int,
        ...
        tx = self.send_from_furynet_to_ethereum(from_fury_addr, to_eth_addr, amount, denom, generate_only=True)
        signed_tx = self.furynode.sign_transaction(tx, from_fury_addr)
        encoded_tx = self.furynode.encode_transaction(signed_tx)
...
class Furygen:
    ...
    # Reference: docker/localnet/furynode/root/scripts/furynode.sh (branch future/peggy2):
    ...
    def create_standalone(self, chainnet, moniker, mnemonic, bind_ip_address, keyring_backend=None):
        ...
        return self.furygen_exec(args, keyring_backend=keyring_backend)

    def furygen_exec(self, args, keyring_backend=None, cwd=None, env=None):
        args = [self.binary] + args + \
...
class Ebrelayer:
    ...
    def init(self, tendermind_node, web3_provider, bridge_registry_contract_address, validator_moniker,
        ...
        args = [self.binary, "init", tendermind_node, web3_provider, bridge_registry_contract_address,
            ...
            (["--gas-prices", fury_format_amount(*gas_prices)] if gas_prices is not None else []) + \
