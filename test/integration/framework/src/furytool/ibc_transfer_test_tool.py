...
# Test environment for testing the new Furynet public SDK: https://docs.google.com/document/d/1MAlg-I0xMnUvbavAZdAN---WuqbyuRyKw-6Lfgfe130/edit
# IBC localnet tool: https://github.com/Furynet/furynet-deploy/commit/0424d2ec81f6f739a3a2cbc77feba1a6f297430d
...
chains = {
    ...
    "furynet": {"binary": "furynoded", "relayer": "ibc"},
}
...
# Generates a furynoded key and stores it into test keyring. Returns the mnemonic that can be used to
# recreate it.
...
def run_tests_for_one_chain_in_one_direction(config, other_chain, direction_flag, number_of_iterations):
    # Assume we're always sending transactions between other_chain and furynet.
    # from_chain is chain sending the assets, to_chain is the receiving chain.
    from_chain = "furynet" if direction_flag else other_chain
    to_chain = other_chain if direction_flag else "furynet"
    ...
    fees = config["fees"]
    gas = config["gas"]
    furynet_proc = start_chain("furynet")
    ...
    mnemonic = add_new_key_to_keyring("furynet", from_account)
