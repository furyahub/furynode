...
import furytool.eth
from furytool import eth, cosmos, command
from furytool.hardhat import Hardhat, default_accounts as hardhat_default_accounts
from furytool.geth import Geth
...
class Integrator(Ganache, Command):
    ...
    def furynoded_peggy2_init_validator(self, furynode, validator_moniker, validator_mnemonic, evm_network_descriptor, validator_power):
        ...
        valoper = furynode.get_val_address(validator_moniker)

        # Add genesis validator
        furynode.add_genesis_validators_peggy(evm_network_descriptor, valoper, validator_power)

    # @TODO Move to Furygen class
    def furygen_create_network(self, chain_id: str, validator_count: int, networks_dir: str, network_definition_file: str,
        seed_ip_address: str, mint_amount: Optional[cosmos.Balance] = None
    ):
        # Old call (no longer works either):
        # furygen network create localnet 1 /mnt/shared/work/projects/fury/furynode/local-tmp/my/deploy/rake/../networks \
        #     192.168.1.2 /mnt/shared/work/projects/fury/furynode/local-tmp/my/deploy/rake/../networks/network-definition.yml \
        #     --keyring-backend file
        # self.cmd.execst(["furygen", "network", "create", "localnet", str(validator_count), networks_dir, seed_ip_address,
        ...
        args = ["furygen", "network", "create", chain_id, str(validator_count), networks_dir, seed_ip_address,
            network_definition_file, "--keyring-backend", "test"] + \
            (["--mint-amount", cosmos.balance_format(mint_amount)] if mint_amount else [])
        self.execst(args)

    # def wait_for_fury_account(self, netdef_json, validator1_address):
    #     # TODO Replace with test_utilities.wait_for_fury_account / wait_for_fury_account_up
    #     return self.execst(["python3", os.path.join(self.project.test_integration_dir, "src/py/wait_for_fury_account.py"),
    ...
    def wait_for_fury_account_up(self, address: cosmos.Address, tcp_url: str = None, timeout: int = 90):
        ...
        #     time.sleep(1)
        # self.wait_for_fury_account(netdef_json, validator1_address)
        ...
        while True:
            try:
                ...
                if last_exception is None:
                    log.debug(f"Waiting for fury account {address}...")
                if time.time() - start_time > timeout:
                    message = "Timeout expired while waiting for fury account {} to become available".format(address)
...
class UIStackEnvironment:
    ...
    def stack_save_snapshot(self):
        ...
        assert shadowfiend_address == self.furynode.keys_show(self.shadowfiend_name)[0]["address"]
        assert akasha_address == self.furynode.keys_show(self.akasha_name)[0]["address"]
        assert juniper_address == self.furynode.keys_show(self.juniper_name)[0]["address"]
        ...
        furynode.add_genesis_account(shadowfiend_address, tokens_shadowfiend)
        furynode.add_genesis_account(akasha_address, tokens_akasha)
        ...
        # ui/scripts/_migrate.sh -> ui/chains/fury/migrate.sh
        # Original scripts say "if we don't sleep there are issues"
        time.sleep(10)
        log.info("Creating liquidity pool from catk:fury...")
        furynode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "catk", [10**5, FURY], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from cbtk:fury...")
        furynode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cbtk", [10**5, FURY], 10**25, 10**25)
        # should now be able to swap from catk:cbtk
        time.sleep(5)
        log.info("Creating liquidity pool from ceth:fury...")
        furynode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ceth", [10**5, FURY], 10**25, 83*10**20)
        # should now be able to swap from x:ceth
        time.sleep(5)
        log.info("Creating liquidity pool from cusdc:fury...")
        furynode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cusdc", [10**5, FURY], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from clink:fury...")
        furynode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "clink", [10**5, FURY], 10**25, 588235*10**18)
        time.sleep(5)
        log.info("Creating liquidity pool from ctest:fury...")
        furynode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ctest", [10**5, FURY], 10**25, 10**13)
    ...
    def stack_push(self):
        ...
        image_root = "ghcr.io/furynet/furynode/ui-stack"
...
class IntegrationTestsEnvironment:
    def __init__(self, cmd):
        ...
        # set_persistant_env_var FURYCHAIN_BIN $BASEDIR/cmd $envexportfile
        ...
        # set_persistant_env_var CONTAINER_NAME integration_furynode1_1 $envexportfile
        ...
        self.network_name = "develop"
        self.network_id = 5777
        self.peruser_storage_dir = self.cmd.get_user_home(".furynode-integration")
        self.state_vars = {}
    ...
    def run(self):
        ...
        # # TODO This should be last (after return from setup_furynet.sh)
        # burn_limits = [
        #     [eth.NULL_ADDRESS, 31*10**18],
        ...
        # test/integration/setup_furynet.sh:
        ...
        # New:
        # furygen network create localnet 1 $NETWORKDIR 192.168.1.2 $NETWORKDIR/network-definition.yml --keyring-backend test \
        ...
        self.cmd.furygen_create_network(chain_id, validator_count, networks_dir, network_definition_file, seed_ip_address, mint_amount=self.mint_amount)
        ...
        relayer_db_path = os.path.join(self.test_integration_dir, "furynetrelayerdb")
        ...
        self.state_vars = {
            ...
            "EBRELAYER_ETHEREUM_PRIVATE_KEY": ebrelayer_ethereum_private_key,  # Needed by furynet_run_ebrelayer.sh
            ...
            "FURYCHAIN_ADMIN_ACCOUNT": adminuser_addr,  # Needed by test_peggy_fees.py (via conftest.py)
            "EBRELAYER_DB": relayer_db_path,  # Created by furynet_run_ebrelayer.sh, does not appear to be used anywhere at the moment
        }
    ...
    def restore_snapshot(self, snapshot_name):
        ...
        assert os.path.realpath(relayer_db_path) == os.path.realpath(os.path.join(self.test_integration_dir, "furynetrelayerdb"))
        extract("ganache.tar.gz", ganache_db_dir)
        extract("furynetrelayerdb.tar.gz", relayer_db_path)
...
class Peggy2Environment(IntegrationTestsEnvironment):
    def __init__(self, cmd: Command, witness_count: int = 1, witness_power: int = 100, consensus_threshold: int = 49,
        ...
        self.chain_id = fury_chain_id
        self.ceth_symbol = furynet_denom_hash(self.ethereum_chain_id, eth.NULL_ADDRESS)
        assert self.ceth_symbol == "furyBridge99990x0000000000000000000000000000000000000000"

        # This goes to validator0, i.e. furynode_validators[0]["address"]
        self.validator_mint_amounts: cosmos.Balance = {
            FURY: 999999 * 10**27,
            self.ceth_symbol: 999999 * 10**21,
            "furyBridge00030x1111111111111111111111111111111111111111": 137 * 10**16,
            "ibc/FEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACE": 137 * 10**16,
        }
        ...
        self.admin_account_mint_amounts: cosmos.Balance = {
            ...
            "furyBridge00030x1111111111111111111111111111111111111111": 10 ** 16,
    ...
    def run(self) -> Tuple[subprocess.Popen, subprocess.Popen, subprocess.Popen, List[subprocess.Popen]]:
        ...
        # - build_golang_binaries before start_furynet
        # - start_hardhat before deploy_smart_contracts
        # - start_furynet before start_witnesses_and_relayers
        ...
        admin_account_name = "furynodeadmin"
        ...
        log.debug("Validator 0 address: {}".format(furynode_validators[0]["address"]))  # mint
        ...
        [relayer0_exec_args], [witness_exec_args] = self.start_witnesses_and_relayers(
            ...
            furynode_validators, furynode_relayers, furynode_witnesses, symbol_translator_file, self.log_level_relayer,
            self.log_level_witness, relayer_extra_args)

        hardhat_scripts.update_validator_power(peggy_sc_addrs["CosmosBridge"], evm_validator_addresses, furynode_witnesses)
        ...
        _unused_peggy2_environment = {
            "admin": {
                ...
                "home": furynode_validator0_home,
            },
            "symbol_translator_file": symbol_translator_file,
            "w3_url": w3_url,
            "evm_chain_id": self.ethereum_chain_id,
            "chain_id": self.chain_id,
            "validators": furynode_validators,  # From yaml file generated by furygen
            "relayers": furynode_relayers,
    ...
    def init_furynet(self, furynoded_network_dir: str, furynoded_log_file: TextIO,
        ...
        res = furynode.peggy2_token_registry_set_registry(registry_json, gas_prices, 1.5, admin_account_address,
            self.chain_id)
        ...
        gas_prices = [0.5, FURY]
        gas_adjustment = 1.5
        furynode.peggy2_set_cross_chain_fee(admin_account_address, self.ethereum_chain_id,
    ...
    def start_witnesses_and_relayers(self, web3_websocket_address: str, hardhat_chain_id: int, tcp_url: str,
        ...
        furynode_validators: Sequence[Mapping[str, Any]], furynode_relayers: Sequence[Mapping[str, Any]],
        furynode_witnesses: Sequence[Mapping[str, Any]], symbol_translator_file: str, log_level_relayer: Optional[str],
        ...
        furynode_validator0 = exactly_one(furynode_validators)
        furynode_validator0_address = furynode_validator0["address"]

        furynode_relayer0 = exactly_one(furynode_relayers)
        furynode_relayer0_mnemonic = furynode_relayer0["name"]
        furynode_relayer0_address = furynode_relayer0["address"]
        furynode_relayer0_home = furynode_relayer0["home"]

        bridge_registry_contract_addr = peggy_sc_addrs["BridgeRegistry"]

        self.cmd.wait_for_fury_account_up(furynode_validator0_address, tcp_url=tcp_url)  # Required for both relayer and witness
        ...
        relayer0_exec_args = ebrelayer.peggy2_build_ebrelayer_cmd(
            ...
            furynode_relayer0_mnemonic,
            chain_id=chain_id,
            node=tcp_url,
            sign_with=furynode_relayer0_address,
            symbol_translator_file=symbol_translator_file,
            ethereum_address=evm_validator0_addr,
            ethereum_private_key=evm_validator0_key,
            keyring_backend="test",
            keyring_dir=furynode_relayer0_home,
            home=furynode_relayer0_home,
            log_level=log_level_relayer,
            **relayer_extra_args,
        )

        witness_exec_args = []
        for i, w in enumerate(furynode_witnesses):
    ...
    def write_env_files(self, project_dir, go_bin_dir, evm_smart_contract_addrs, eth_accounts, admin_account_name,
        ...
        def format_fury_account(fury_account):
            return {"account": fury_account["address"], "name": fury_account["name"], "homeDir": fury_account["home"]}
        ...
        environment_json = {
            ...
            "furyResults": {
                "validatorValues": furynode_validators,
                "adminAddress": format_fury_account({"address": admin_account_address, "name": admin_account_name, "home": furynode_validator0_home}),
                "relayerAddresses": [format_fury_account(a) for a in furynode_relayers],
                "witnessAddresses": [format_fury_account(a) for a in furynode_witnesses],
                # "process": { ... },
                "tcpurl": tcp_url,
            }
        }
        ...
        dot_env = dict_merge({
            ...
            "GOBIN": go_bin_dir,
            "TCP_URL": tcp_url,
            "VALIDATOR_ADDRESS": furynode_validators[0]["address"],
            "VALIDATOR_CONSENSUS_ADDRESS": furynode_validators[0]["validator_consensus_address"],
            "VALIDATOR_MENOMONIC": furynode_validators[0]["mnemonic"],
            "VALIDATOR_MONIKER": furynode_validators[0]["moniker"],
            "VALIDATOR_PASSWORD": furynode_validators[0]["password"],
            "VALIDATOR_PUB_KEY": furynode_validators[0]["pub_key"],
        ...
        } for i, validator in enumerate(furynode_validators)], *[{
            f"RELAYER_ADDRESS_{i}": relayer["address"],
            f"RELAYER_NAME_{i}": relayer["name"],
        } for i, relayer in enumerate(furynode_relayers)], *[{
        ...
        launch_json = {
            ...
            "configurations": [
                {
                    "name": "Python: furytool",
                    "type": "python",
                    "request": "launch",
                    "program": "${workspaceFolder}/test/integration/framework/furytool",
                    "args": ["run-env"],
...
class IBCEnvironment(IntegrationTestsEnvironment):
    ...
    def run(self):
        chainnet0 = "furynet-ibc-0"
        chainnet1 = "furynet-ibc-1"
        ...
        furygen = Furygen(self.cmd)
        # This does not work - "--keyring-backend" is not supported
        x = furygen.create_standalone(chainnet0, "chain1", mnemonic, ipaddr0, keyring_backend=None)
