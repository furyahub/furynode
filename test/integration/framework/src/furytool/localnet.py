...
from furytool.common import project_dir, furytool_logger
...
def run_localnet_hook():
    ...
    #
    # if furynet: ${binPath}/${binary} set-gen-denom-whitelist ${home}/config/denoms.json --home ${home}
    #
...
def download_ibc_binaries(cmd, chains_to_download=None, output_path=None):
    ...
    config = get_localnet_config(cmd)
    tmpdir = cmd.mktempdir()
    # We prefer to compile furynet. Sentinel uses sourceUrl, but there is no Makefile.
    all_supported_chains = set(config.keys()).difference({"furynet", "sentinel"})
