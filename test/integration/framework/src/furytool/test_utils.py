...
from furytool import eth, truffle, hardhat, run_env, furynet, cosmos, command
...
furynode_funds_for_transfer_peggy1 = 10**17  # fury
...
def get_env_ctx(cmd=None, env_file=None, env_vars=None):
    ...
    # Add any Furynet private keys to test keystore
    fury_user_private_keys = ctx.cmd.project.read_peruser_config_file("fury-keys")
    if fury_user_private_keys:
        available_fury_accounts = ctx.furynode.keys_list()
        for name, address, mnemonic in [[e["name"], e["address"], e["mnemonic"].split(" ")] for e in fury_user_private_keys]:
            existing_acct = [a for a in available_fury_accounts if a["address"] == address]
            if not existing_acct:
                acct = ctx.furynode.keys_add(name, mnemonic)
                assert acct["address"] == address, "Invalid address for fury account {}".format(name)
    return ctx

def get_env_ctx_peggy2():
    cmd = run_env.Integrator()

    if "FURYTOOL_ENV_FILE" in os.environ:
        # New-style format (hopefully unified)
        # This is for connecting to peggy2-tempnet etc.
        env_file = os.environ["FURYTOOL_ENV_FILE"]
        ...
        fury_source = furynet_config.get("fury_source")
        if not fury_source:
            if "fury_source_mnemonic" in furynet_config:
                fury_source_mnemonic = furynet_config["fury_source_mnemonic"]
        ...
        # For either `furytool run-env` or `devenv`
    ...
    ceth_symbol = furynet.furynet_denom_hash(ethereum_network_descriptor, eth.NULL_ADDRESS)
...
def get_env_ctx_peggy1(cmd=None, env_file=None, env_vars=None):
    ...
    if "CHAINNET" in env_vars:
        furynode_chain_id = env_vars["CHAINNET"]
    elif deployment_name:
        furynode_chain_id = deployment_name
    else:
        furynode_chain_id = "localnet"
    ...
    generic_erc20_contract_name = "FurynetTestToken"
    if "SMART_CONTRACT_ARTIFACT_DIR" in env_vars:
        ...
        if deployment_name == "furynet-1":
            # Special case for Betanet because FurynetTestToken is not deployed there.
...
def fury_addr_to_evm_arg(fury_address):
    return fury_address.encode("UTF-8")
...
class EnvCtx:
    def __init__(self, cmd: command.Command, w3_conn: web3.Web3, ctx_eth: eth.EthereumTxWrapper, abi_provider,
        ...
        self.furynet_ethbridge_admin_account = self.fury_source
    ...
    def tx_deploy_new_generic_erc20_token(self, deployer_addr: str, name: str, symbol: str, decimals: int, cosmosDenom: str = None) -> Contract:
        # return self.tx_deploy("FurynetTestToken", self.operator, [name, symbol, decimals])
        if on_peggy2_branch:
            # Use BridgeToken
            assert self.generic_erc20_contract == "BridgeToken"
            if cosmosDenom is None:
                cosmosDenom = "erc20denom"  # TODO Dummy variable since we're using BridgeToken instead of FurynetTestToken

            constructor_args = [name, symbol, decimals, cosmosDenom]
        else:
            # Use FurynetTestToken for TestNet and Devnet, and BridgeToken for Betanet
    ...
    def tx_bridge_bank_lock_eth(self, from_eth_acct, to_fury_acct, amount):
        recipient = fury_addr_to_evm_arg(to_fury_acct)
        bridge_bank = self.get_bridge_bank_sc()
    ...
    def tx_bridge_bank_lock_erc20(self, token_addr, from_eth_acct, to_fury_acct, amount):
        recipient = fury_addr_to_evm_arg(to_fury_acct)
        bridge_bank = self.get_bridge_bank_sc()
    ...
    def tx_bridge_bank_burn_erc20(self, token_addr: str, from_eth_acct: str, to_fury_acct: str, amount: int) -> HexBytes:
        recipient = fury_addr_to_evm_arg(to_fury_acct)
        bridge_bank = self.get_bridge_bank_sc()
        # When transfering ERC20, the amount needs to be passed as argument, and the "message.value" should be 0
        tx_opts = {"value": 0}
    ...
    def tx_approve_and_lock(self, token_sc, from_eth_acct, to_fury_acct, amount):
        bridge_bank_sc = self.get_bridge_bank_sc()
        txhash1 = self.tx_approve(token_sc, self.operator, bridge_bank_sc.address, amount)
        txhash2 = self.tx_bridge_bank_lock_erc20(token_sc.address, from_eth_acct, to_fury_acct, amount)
        log.debug("tx_approve_and_lock: {} '{}' ({}) from {} to {}".format(amount, token_sc.functions.name().call(),
            token_sc.functions.symbol().call(), from_eth_acct, to_fury_acct))
    ...
    def create_new_currency(self, symbol, name, decimals, amount, minted_tokens_recipient):
        ...
        1. Deploys a new instance of FurynetTestToken
        ...
        """
        assert self.generic_erc20_contract == "FurynetTestToken"  # Preserve compatibiliy with integration tests and inflate_tokens.sh
    ...
    def send_eth_from_ethereum_to_furynet(self, from_eth_addr, to_fury_addr, amount):
        # recipient = to_fury_addr.encode("UTF-8")
    ...
    # TODO Obsolete, use self.bridge_bank_lock_eth()
    def send_erc20_from_ethereum_to_furynet(self, from_eth_addr, dest_sichain_addr, erc20_token_addr, amount):
    ...
    def create_furynet_addr(self, moniker: str = None,
        ...
        """
        Generates a new furynet address in test keyring. If moniker is given, uses it, otherwise
        generates a random one 'test-xxx'. If fund_amounts is given, the furynet funds are transferred
        from fury_source to the account before returning.
        """
        moniker = moniker or "test-" + random_string(20)
        acct = self.furynode.keys_add(moniker)
        fury_address = acct["address"]
        if fund_amounts:
            fund_amounts = cosmos.balance_normalize(fund_amounts)  # Convert from old format if neccessary
            fury_source_balances = self.get_furynet_balance(self.fury_source)
            for denom, required_amount in fund_amounts.items():
                ...
                assert available_amount >= required_amount, "Fury source {} would need {}, but only has {}".format(
                    self.fury_source, fury_format_amount(required_amount, denom), fury_format_amount(available_amount, denom))
            old_balances = self.get_furynet_balance(fury_address)
            self.send_from_furynet_to_furynet(self.fury_source, fury_address, fund_amounts)
            self.furynode.wait_for_balance_change(fury_address, old_balances, min_changes=fund_amounts)
            new_balances = self.get_furynet_balance(fury_address)
            assert cosmos.balance_zero(cosmos.balance_sub(new_balances, fund_amounts))
        return fury_address

    # TODO Clean up
    def send_from_furynet_to_furynet(self, from_fury_addr: cosmos.Address, to_fury_addr: cosmos.Address,
        amounts: cosmos.Balance
    ) -> Mapping:
        return self.furynode.send(from_fury_addr, to_fury_addr, amounts)

    # TODO Clean up
    def get_furynet_balance(self, fury_addr: cosmos.Address, height: Optional[int] = None,
        disable_log: bool = False, retries_on_error: Optional[int] = None, delay_on_error: int = 3
    ) -> cosmos.Balance:
        return self.furynode.get_balance(fury_addr, height=height, disable_log=disable_log,
            retries_on_error=retries_on_error, delay_on_error=delay_on_error)

    # TODO Clean up
    def get_current_block(self):
        return self.furynode.get_current_block()

    # TODO Clean up
    def status(self):
        return self.furynode.status()

    def eth_symbol_to_fury_symbol(self, eth_token_symbol):
        assert not on_peggy2_branch
        # TODO furynet.use furynet_denom_hash() if on_peggy2_branch
        # E.g. "usdt" -> "cusdt"
    ...
    def bridge_bank_lock_eth(self, from_eth_acct, to_fury_acct, amount):
        """ Sends ETH from Ethereum to Furynet (lock) """
        txhash = self.tx_bridge_bank_lock_eth(from_eth_acct, to_fury_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)

    def bridge_bank_lock_erc20(self, token_sc, from_eth_acct, to_fury_acct, amount):
        txhash = self.tx_bridge_bank_lock_erc20(token_sc.address, from_eth_acct, to_fury_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)

    def bridge_bank_burn_erc20(self, token_sc: Contract, from_eth_acct: str, to_fury_acct: str, amount: int) -> TxReceipt:
        txhash = self.tx_bridge_bank_burn_erc20(token_sc.address, from_eth_acct, to_fury_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)
    ...
    # For Furynet-native assets (fury) we need to use "burn".
    # Compare: smart-contracts/scripts/test/{sendLockTx.js OR sendBurnTx.js}
    # sendBurnTx is called when furynet_symbol == "fury", sendLockTx otherwise
    def send_from_ethereum_to_furynet(self, from_eth_acct: str, to_fury_acct: str, amount: int, token_sc: Contract = None, isLock: bool = True) -> TxReceipt:
        if token_sc is None:
            # ETH transfer
            self.bridge_bank_lock_eth(from_eth_acct, to_fury_acct, amount)
        else:
            # ERC20 token transfer
            self.approve_erc20_token(token_sc, from_eth_acct, amount)
            if isLock:
                self.bridge_bank_lock_erc20(token_sc, from_eth_acct, to_fury_acct, amount)
            else:
                self.bridge_bank_burn_erc20(token_sc, from_eth_acct, to_fury_acct, amount)
    ...
    def sanity_check(self):
        ...
        if on_peggy2_branch:
            ...
            assert (self.furynode.chain_id != "furynet-testnet-1") or (bridge_bank_sc.address == "0x6CfD69783E3fFb44CBaaFF7F509a4fcF0d8e2835")
            assert (self.furynode.chain_id != "furynet-devnet-1") or (bridge_bank_sc.address == "0x96DC6f02C66Bbf2dfbA934b8DafE7B2c08715A73")
            assert (self.furynode.chain_id != "localnet") or (bridge_bank_sc.address == "0x30753E4A8aad7F8597332E813735Def5dD395028")
        ...
        available_accounts = self.furynode.keys_list()
        ...
        if len(fury_source_account) != 1:
            raise Exception
        fury_source_balance = self.get_furynet_balance(self.fury_source).get(FURY, 0)
