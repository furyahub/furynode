...
class Command:
    ...
    # Starts a process asynchronously (for furynoded, hardhat, ebrelayer etc.)
    ...
    def mktempdir(self, parent_dir: Optional[str] = None) -> str:
        ...
        args = ["mktemp", "-d", "-p", parent_dir] if not self.is_mac() else ["mktemp", "-d", os.path.join(parent_dir, "furytool.XXXXXX")]
        return exactly_one(stdout_lines(self.execst(args)))

    def mktempfile(self, parent_dir: Optional[str] = None) -> str:
        parent_dir = parent_dir or self.tmpdir()
        args = ["mktemp", "-p", parent_dir] if not self.is_mac() else ["mktemp", os.path.join(parent_dir, "furytool.XXXXXX")]
        return exactly_one(stdout_lines(self.execst(args)))
