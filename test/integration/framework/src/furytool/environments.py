...
from furytool.furynet import FURY, FURY_DECIMALS, STAKE
from furytool import furynet, command, cosmos


# Environment for load test test_many_pools_and_liquidity_providers and for testing min commission/max voting power
# Just furynode, no ethereum
# Multi-node support
class FurynodedEnvironment:
    def __init__(self, cmd: command.Command, chain_id: Optional[str] = None, furynoded_home_root: Optional[str] = None):
        self.cmd = cmd
        self.furynoded_home_root = furynoded_home_root if furynoded_home_root is not None else cmd.mktempdir()
        self.keyring_dir = os.path.join(self.furynoded_home_root, "keyring")
        self.chain_id = chain_id or "localnet"
        self.staking_denom = FURY
        self.default_binary = "furynoded"
        ...
        self.open_log_files = []
        self._state = 0
        self.furynoded = furynet.Furynoded(self.cmd, home=self.keyring_dir, chain_id=self.chain_id)
    ...
    def add_validator(self, /,  binary: Optional[str] = None, admin_name: Optional[str] = None,
        ...
        moniker = moniker if moniker is not None else "furynoded-{}".format(next_id)
        home = home if home is not None else os.path.join(self.furynoded_home_root, moniker)
        ...
        log_file = log_file if log_file is not None else os.path.join(home, "furynoded.log")
        ...
        if self._state == 2:
            ...
            furynoded = self._furynoded_for(self.node_info[0])
            furynoded_i = self._furynoded_for(node_info)
            furynoded_i.save_genesis_json(furynoded.load_genesis_json())
            self._furynoded_start(node_info)
    ...
    def init(self, faucet_balance: Optional[cosmos.Balance] = None, extra_accounts: Optional[cosmos.Bank] = None,
        ...
        self.faucet = self.furynoded.create_addr("faucet")
        ...
        furynoded0 = self._furynoded_for(node_info0)

        for node_info in self.node_info:
            furynoded = self._furynoded_for(node_info)
            admin_addr = node_info["admin_addr"]
            admin_bech = furynoded.get_val_address(admin_addr)
            validator_balance = cosmos.balance_add({self.staking_denom: node_info["staking_amount"]}, node_info["initial_balance"])
            furynoded0.add_genesis_account(admin_addr, validator_balance)
            furynoded0.add_genesis_validators(admin_bech)

        admin0_addr = node_info0["admin_addr"]
        admin0_name = node_info0["admin_name"]
        self.clp_admin = admin0_addr
        furynoded0.add_genesis_clp_admin(admin0_addr)
        furynoded0.set_genesis_oracle_admin(admin0_name)
        furynoded0.set_genesis_whitelister_admin(admin0_name)
        ...
        genesis = furynoded0.load_genesis_json()
        ...
        if extra_genesis_balances:
            furynoded0.add_accounts_to_existing_genesis(genesis, extra_genesis_balances)
        furynoded0.save_genesis_json(genesis)
    ...
    def start(self):
        ...
        furynoded0 = self._furynoded_for(node_info)
        admin0_name = node_info["admin_name"]
        staking_amount = {self.staking_denom: node_info["staking_amount"]}

        furynoded0.gentx(admin0_name, staking_amount, keyring_dir=self.keyring_dir,
        ...
        furynoded0.collect_gentx()
        furynoded0.validate_genesis()
        ...
        for node_info in other_validators:
            furynoded = self._furynoded_for(node_info)
            furynoded.save_genesis_json(genesis)

        for node_info in self.node_info:
            log_file, process = self._furynoded_start(node_info)
        ...
        # wallets (symptom: "furynoded status" and "furynoded query" would work, but "furynoded tx" would get
        # "Error: post failed: Post "http://...": EOF".
        furynoded0.wait_for_last_transaction_to_be_mined()
        furynoded0.wait_for_last_transaction_to_be_mined()
        ...
        self.furynoded = furynet.Furynoded(self.cmd, home=self.keyring_dir, chain_id=self.chain_id,
            node=furynet.format_node_url(self.node_info[0]["host"], self.node_info[0]["ports"]["rpc"]),
    ...
    def fund(self, address: cosmos.Address, amounts: cosmos.Balance):
        ...
        self.furynoded.send_and_check(self.faucet, address, amounts)
    ...
    def upgrade(self, new_version: str, new_binary: str, upgrade_height: int, deposit_amount: Optional[int] = None):
        ...
        # upgrade_height = env.furynoded[0].get_current_block() + 15  # Note: must be > 60s (as per app config)

        proposals_before = furynoded.query_gov_proposals()
        res = furynoded.gov_submit_software_upgrade(new_version, admin_addr, deposit, upgrade_height, upgrade_info,
            "test_release", "Test Release", broadcast_mode="block")
        furynet.check_raw_log(res)
        furynoded.wait_for_last_transaction_to_be_mined()
        proposals_after = furynoded.query_gov_proposals()
        ...
        res = furynoded.gov_vote(proposal_id, True, admin_addr, broadcast_mode="block")
        furynet.check_raw_log(res)

        furynoded.wait_for_block(upgrade_height)
        time.sleep(5)
        self.close()
        time.sleep(5)
        for node_info in self.node_info:
        ...
        furynoded = self._furynoded_for(node_info)  # Probably we could still use an older version of the client, but let's be consistent
        assert furynoded.version() == new_version
    ...
    def _update_configuration_files(self, node_info, peers_node_info):
        ...
        peers = [furynet.format_peer_address(i["node_id"], LOCALHOST, i["ports"]["p2p"])
            for i in peers_node_info]
        app_toml = furynoded.load_app_toml()
        config_toml = furynoded.load_config_toml()
        app_toml["minimum-gas-prices"] = fury_format_amount(0.5, FURY)
        app_toml['api']['enable'] = True
        app_toml["api"]["address"] = furynet.format_node_url(ANY_ADDR, node_info["ports"]["api"])
        ...
        furynoded.save_app_toml(app_toml)
        furynoded.save_config_toml(config_toml)

    # This constructs a furynoded CLI wrapper with values for --home, --chain_id and --node taken from corresponding
    ...
    def _furynoded_for(self, node_info: JsonDict, to_node_info: Optional[JsonDict] = None) -> furynet.Furynoded:
        binary = node_info["binary"]
        home = node_info["home"]
        to_node_info = to_node_info if to_node_info is not None else node_info
        node = furynet.format_node_url(to_node_info["host"], to_node_info["ports"]["rpc"])
        return furynet.Furynoded(self.cmd, binary=binary, home=home, chain_id=self.chain_id, node=node)

    def _furynoded_start(self, node_info: JsonDict):
        furynoded = self._furynoded_for(node_info)
        ports = node_info["ports"]
        log_file_path = node_info["log_file"]
        log_level = node_info["log_level"]
        log_file = open(log_file_path, "w")
        process = furynoded.furynoded_start(log_file=log_file, log_level=log_level, trace=True,
    ...
    def _broadcast_create_validator_msg(self, node_info: JsonDict):
        ...
        # Send "create validator" transaction. For this we need to use furynoded with new validator's keystore, but with
        ...
        furynoded_tmp = self._furynoded_for(node_info, to_node_info=self.node_info[0])

        validators_before = furynoded_tmp.query_staking_validators()
        assert moniker not in validators_before

        admin_balance = furynoded_tmp.get_balance(admin_addr)
        assert cosmos.balance_exceeds(admin_balance, {FURY: furynet.fury_tx_fee_in_fury}), \
            "Validator admin {} needs at least one fury_tx_fee_in_fury to fund the transaction".format(admin_addr)
        assert cosmos.balance_exceeds(admin_balance, stake), \
            "Validator needs at least {} for staking".format(cosmos.balance_format(stake))

        res = furynoded_tmp.staking_create_validator(stake, pubkey, moniker, commission_rate, commission_max_rate,
        ...
        validators_after = furynoded_tmp.query_staking_validators()
    ...
    def _create_validator_home(self, node_info: JsonDict):
        ...
        furynoded = furynet.Furynoded(self.cmd, home=self.keyring_dir)
        admin_acct, admin_mnemonic = furynoded._keys_add(admin_name, mnemonic=admin_mnemonic)
        admin_addr = admin_acct["address"]
        node_info["admin_addr"] = admin_addr

        furynoded_i = self._furynoded_for(node_info)
        moniker = node_info["moniker"]
        furynoded_i.init(moniker)
        admin_account_copy = furynoded_i.keys_add(admin_name, mnemonic=admin_mnemonic)
        assert admin_account_copy["address"] == admin_addr
        node_id = furynoded_i.tendermint_show_node_id()  # Taken from ${furynoded_home}/config/node_key.json
        pubkey = furynoded_i.tendermint_show_validator()  # Taken from ${furynoded_home}/config/priv_validator_key.json
    ...
    def setup_liquidity_pools_simple(self, pool_definitions: Mapping[str, Tuple[int, int, int]]):
        ...
        assert len(furynoded.query_pools()) == 0  # This method is single-shot for now
        ...
        if self.faucet != self.clp_admin:
            ...
            furynoded.send_batch(self.faucet, self.clp_admin, cosmos.balance_add(
                {denom: external_amount for denom, (_, _, external_amount) in pool_definitions.items()},
                {furynet.FURY: sum(native_amount for _, (_, native_amount, _) in pool_definitions.items())}))
        ...
        # "token not supported by furynet"
        # Note: original_entry = {
        ...
        #     "permissions": [1]
        # }
        furynoded.token_registry_register(furynoded.create_tokenregistry_entry(FURY, FURY, FURY_DECIMALS, fury_permissions),
            self.clp_admin, broadcast_mode="block")

        furynoded.token_registry_register_batch(self.clp_admin,
            [furynoded.create_tokenregistry_entry(denom, denom, decimals, other_permissions) for denom, (decimals, _, _) in pool_definitions.items()])
        furynoded.create_liquidity_pools_batch(self.clp_admin,
            [(denom, native_amount, external_amount) for denom, (_, native_amount, external_amount) in pool_definitions.items()])

        assert len(self.furynoded.query_pools()) == len(pool_definitions)
