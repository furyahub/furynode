...
# Multi-node support
class FuryndEnvironment:
    def __init__(self, cmd: command.Command, chain_id: Optional[str] = None, furynd_home_root: Optional[str] = None):
        self.cmd = cmd
        self.furynd_home_root = furynd_home_root if furynd_home_root is not None else cmd.mktempdir()
        self.keyring_dir = os.path.join(self.furynd_home_root, "keyring")
        self.chain_id = chain_id or "localnet"
        self.staking_denom = FURY
        self.default_binary = "furynd"
        ...
        self.open_log_files = []
        self._state = 0
        self.furynd = furynet.Furynd(self.cmd, home=self.keyring_dir, chain_id=self.chain_id)
    ...
    def add_validator(self, /,  binary: Optional[str] = None, admin_name: Optional[str] = None,
        ...
        moniker = moniker if moniker is not None else "furynd-{}".format(next_id)
        home = home if home is not None else os.path.join(self.furynd_home_root, moniker)
        ...
        log_file = log_file if log_file is not None else os.path.join(home, "furynd.log")
        ...
        if self._state == 2:
            ...
            furynd = self._furynd_for(self.node_info[0])
            furynd_i = self._furynd_for(node_info)
            furynd_i.save_genesis_json(furynd.load_genesis_json())
            self._furynd_start(node_info)
    ...
    def init(self, faucet_balance: Optional[cosmos.Balance] = None, extra_accounts: Optional[cosmos.Bank] = None,
        ...
        self.faucet = self.furynd.create_addr("faucet")
        ...
        furynd0 = self._furynd_for(node_info0)

        for node_info in self.node_info:
            furynd = self._furynd_for(node_info)
            admin_addr = node_info["admin_addr"]
            admin_bech = furynd.get_val_address(admin_addr)
            validator_balance = cosmos.balance_add({self.staking_denom: node_info["staking_amount"]}, node_info["initial_balance"])
            furynd0.add_genesis_account(admin_addr, validator_balance)
            furynd0.add_genesis_validators(admin_bech)

        admin0_addr = node_info0["admin_addr"]
        admin0_name = node_info0["admin_name"]
        self.clp_admin = admin0_addr
        furynd0.add_genesis_clp_admin(admin0_addr)
        furynd0.set_genesis_oracle_admin(admin0_name)
        furynd0.set_genesis_whitelister_admin(admin0_name)
        ...
        genesis = furynd0.load_genesis_json()
        ...
        if extra_genesis_balances:
            furynd0.add_accounts_to_existing_genesis(genesis, extra_genesis_balances)
        furynd0.save_genesis_json(genesis)
    ...
    def start(self):
        ...
        furynd0 = self._furynd_for(node_info)
        admin0_name = node_info["admin_name"]
        staking_amount = {self.staking_denom: node_info["staking_amount"]}

        furynd0.gentx(admin0_name, staking_amount, keyring_dir=self.keyring_dir,
        ...
        furynd0.collect_gentx()
        furynd0.validate_genesis()
        ...
        for node_info in other_validators:
            furynd = self._furynd_for(node_info)
            furynd.save_genesis_json(genesis)

        for node_info in self.node_info:
            log_file, process = self._furynd_start(node_info)
        ...
        # wallets (symptom: "furynd status" and "furynd query" would work, but "furynd tx" would get
        # "Error: post failed: Post "http://...": EOF".
        furynd0.wait_for_last_transaction_to_be_mined()
        furynd0.wait_for_last_transaction_to_be_mined()
    ...
    def fund(self, address: cosmos.Address, amounts: cosmos.Balance):
        ...
        self.furynd.send_and_check(self.faucet, address, amounts)
    ...
    def upgrade(self, new_version: str, new_binary: str, upgrade_height: int, deposit_amount: Optional[int] = None):
        ...
        # upgrade_height = env.furynd[0].get_current_block() + 15  # Note: must be > 60s (as per app config)

        proposals_before = furynd.query_gov_proposals()
        res = furynd.gov_submit_software_upgrade(new_version, admin_addr, deposit, upgrade_height, upgrade_info,
            "test_release", "Test Release", broadcast_mode="block")
        furynet.check_raw_log(res)
        furynd.wait_for_last_transaction_to_be_mined()
        proposals_after = furynd.query_gov_proposals()
        ...
        res = furynd.gov_vote(proposal_id, True, admin_addr, broadcast_mode="block")
        furynet.check_raw_log(res)

        furynd.wait_for_block(upgrade_height)
        time.sleep(5)
        self.close()
        time.sleep(5)
        for node_info in self.node_info:
        ...
        furynd = self._furynd_for(node_info)  # Probably we could still use an older version of the client, but let's be consistent
        assert furynd.version() == new_version
    ...
    def _update_configuration_files(self, node_info, peers_node_info):
        ...
        app_toml = furynd.load_app_toml()
        config_toml = furynd.load_config_toml()
        ...
        furynd.save_app_toml(app_toml)
        furynd.save_config_toml(config_toml)

    # This constructs a furynd CLI wrapper with values for --home, --chain_id and --node taken from corresponding
    ...
    def _furynd_for(self, node_info: JsonDict, to_node_info: Optional[JsonDict] = None) -> furynet.Furynd:
        ...
        return furynet.Furynd(self.cmd, binary=binary, home=home, chain_id=self.chain_id, node=node)

    def _furynd_start(self, node_info: JsonDict):
        furynd = self._furynd_for(node_info)
        ports = node_info["ports"]
        log_file_path = node_info["log_file"]
        log_level = node_info["log_level"]
        log_file = open(log_file_path, "w")
        process = furynd.furynd_start(log_file=log_file, log_level=log_level, trace=True,
    ...
    def _broadcast_create_validator_msg(self, node_info: JsonDict):
        ...
        # Send "create validator" transaction. For this we need to use furynd with new validator's keystore, but with
        ...
        furynd_tmp = self._furynd_for(node_info, to_node_info=self.node_info[0])

        validators_before = furynd_tmp.query_staking_validators()
        assert moniker not in validators_before

        admin_balance = furynd_tmp.get_balance(admin_addr)
        ...
        res = furynd_tmp.staking_create_validator(stake, pubkey, moniker, commission_rate, commission_max_rate,
        ...
        validators_after = furynd_tmp.query_staking_validators()
    ...
    def _create_validator_home(self, node_info: JsonDict):
        ...
        furynd = furynet.Furynd(self.cmd, home=self.keyring_dir)
        admin_acct, admin_mnemonic = furynd._keys_add(admin_name, mnemonic=admin_mnemonic)
        admin_addr = admin_acct["address"]
        node_info["admin_addr"] = admin_addr

        furynd_i = self._furynd_for(node_info)
        moniker = node_info["moniker"]
        furynd_i.init(moniker)
        admin_account_copy = furynd_i.keys_add(admin_name, mnemonic=admin_mnemonic)
        assert admin_account_copy["address"] == admin_addr
        node_id = furynd_i.tendermint_show_node_id()  # Taken from ${furynd_home}/config/node_key.json
        pubkey = furynd_i.tendermint_show_validator()  # Taken from ${furynd_home}/config/priv_validator_key.json
    ...
    def setup_liquidity_pools_simple(self, pool_definitions: Mapping[str, Tuple[int, int, int]]):
        assert self._state == 2
        furynd = self.furynd
        assert len(furynd.query_pools()) == 0  # This method is single-shot for now
        ...
        if self.faucet != self.clp_admin:
            ...
            furynd.send_batch(self.faucet, self.clp_admin, cosmos.balance_add(
        ...
        #     "permissions": [1]
        # }
        furynd.token_registry_register(furynd.create_tokenregistry_entry(FURY, FURY, FURY_DECIMALS, fury_permissions),
            self.clp_admin, broadcast_mode="block")

        furynd.token_registry_register_batch(self.clp_admin,
            [furynd.create_tokenregistry_entry(denom, denom, decimals, other_permissions) for denom, (decimals, _, _) in pool_definitions.items()])
        furynd.create_liquidity_pools_batch(self.clp_admin,
            [(denom, native_amount, external_amount) for denom, (_, native_amount, external_amount) in pool_definitions.items()])

        assert len(self.furynd.query_pools()) == len(pool_definitions)
