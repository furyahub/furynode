...
# TODO Move to furynet.py
# TODO Refactoring in progress. This should be moved to furynet.py and only used for gas (float amount) + renamed
def fury_format_amount(amount: Union[int, float], denom: str) -> str:
...
def furytool_logger(name: Optional[str] = None):
    # Shortening "furytool.eth" to "eth" results in a name clash with "eth" dependencies for which we want to disable logging...
    # if name is not None:
...
__log = furytool_logger(__name__)
