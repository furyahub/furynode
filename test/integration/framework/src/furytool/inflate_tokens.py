...
# The original script had a lot of problems as described in https://app.zenhub.com/workspaces/current-sprint---engineering-615a2e9fe2abd5001befc7f9/issues/furynet/issues/719.
# See https://www.notion.so/furynet/TEST-TOKEN-DISTRIBUTION-PROCESS-41ad0861560c4be58918838dbd292497

import json
import re
from typing import Any, Mapping, Iterable, Sequence

from furytool import eth, test_utils, cosmos
...
class InflateTokens:
    ...
    def get_whitelisted_tokens(self) -> List[TokenDict]:
        ...
        for token_addr, value in whitelist.items():
            ...
            token = {
                ...
                "fury_denom": self.ctx.eth_symbol_to_fury_symbol(token_symbol),
            }
    ...
    def build_list_of_tokens_to_create(self, existing_tokens: Iterable[TokenDict], requested_tokens: Iterable[TokenDict]
        ...
        """
        This part deploys FurynetTestoken for every requested token that has not yet been deployed.
    ...
    def transfer_from_eth_to_furynode(self, from_eth_addr, to_fury_addr, tokens_to_transfer, amount_in_tokens, amount_eth_gwei):
        fury_balances_before = self.ctx.get_furynet_balance(to_fury_addr)
        sent_amounts = []
        pending_txs = []
        for token in tokens_to_transfer:
            ...
            token_sc = self.ctx.get_generic_erc20_sc(token_addr)
            amount = amount_in_tokens * 10**decimals
            pending_txs.extend(self.ctx.tx_approve_and_lock(token_sc, from_eth_addr, to_fury_addr, amount))
            sent_amounts.append([amount, token["fury_denom"]])
        if amount_eth_gwei > 0:
            amount = amount_eth_gwei * eth.GWEI
            pending_txs.append(self.ctx.tx_bridge_bank_lock_eth(from_eth_addr, to_fury_addr, amount))
        ...
        # Wait for intermediate_fury_account to receive all funds across the bridge
        ...
        self.ctx.furynode.wait_for_balance_change(to_fury_addr, fury_balances_before, min_changes=sent_amounts,
            polling_time=5, timeout=0, change_timeout=self.wait_for_account_change_timeout)

    # Distributes from intermediate_fury_account to each individual account
    def distribute_tokens_to_wallets(self, from_fury_account, tokens_to_transfer, amount_in_tokens, target_fury_accounts, amount_eth_gwei):
        send_amounts = [[amount_in_tokens * 10**t["decimals"], t["fury_denom"]] for t in tokens_to_transfer]
        if amount_eth_gwei > 0:
            send_amounts.append([amount_eth_gwei * eth.GWEI, self.ctx.ceth_symbol])
        progress_total = len(target_fury_accounts) * len(send_amounts)
        progress_current = 0
        for fury_acct in target_fury_accounts:
            remaining = send_amounts
            while remaining:
                batch_size = len(remaining)
                if (self.ctx.furynode.max_send_batch_size > 0) and (batch_size > self.ctx.furynode.max_send_batch_size):
                    batch_size = self.ctx.furynode.max_send_batch_size
                batch = remaining[:batch_size]
                remaining = remaining[batch_size:]
                fury_balance_before = self.ctx.get_furynet_balance(fury_acct)
                self.ctx.send_from_furynet_to_furynet(from_fury_account, fury_acct, batch)
                self.ctx.furynode.wait_for_balance_change(fury_acct, fury_balance_before, min_changes=batch,
    ...
    def transfer(self, requested_tokens: Sequence[TokenDict], token_amount: int,
        target_fury_accounts: Sequence[cosmos.Address], eth_amount_gwei: int
    ):
        """
        It goes like this:
        ...
        3. It will do a single transaction across the bridge to move all tokens from OPERATOR to fury_broker_account
        4. It will distribute tokens from fury_broker_account to each of given target accounts
        The fury_broker_account and OPERATOR can be any Furynet and Ethereum accounts, we might want to use something
        ...
        n_accounts = len(target_fury_accounts)
        ...
        number_of_batches = 1 if self.ctx.furynode.max_send_batch_size == 0 else (len(requested_tokens) + 1) // self.ctx.furynode.max_send_batch_size + 1
        fund_fury = [5 * test_utils.furynode_funds_for_transfer_peggy1 * n_accounts * number_of_batches, "fury"]
        log.debug("Estimated number of batches needed to transfer tokens from intermediate fury account to target fury wallet: {}".format(number_of_batches))
        log.debug("Estimated fury funding needed for intermediate account: {}".format(fund_fury))
        ether_faucet_account = self.ctx.operator
        fury_broker_account = self.ctx.create_furynet_addr(fund_amounts=[fund_fury])
        ...
        log.info("Using fury_broker_account {}".format(fury_broker_account))

        # Check first that we have the key for FURY_SOURCE since the script uses it as an intermediate address
        keys = self.ctx.furynode.keys_list()
        fury_source_key = zero_or_one([k for k in keys if k["address"] == fury_broker_account])
        assert fury_source_key is not None, "Need private key of broker account {} in furynoded test keyring".format(fury_broker_account)
        ...
        if (self.max_ethereum_batch_size > 0) and (len(tokens_to_transfer) > self.max_ethereum_batch_size):
            log.debug(f"Transferring {len(tokens_to_transfer)} tokens from ethereum to furyndde in batches of {self.max_ethereum_batch_size}...")
            remaining = tokens_to_transfer
            while remaining:
                batch = remaining[:self.max_ethereum_batch_size]
                remaining = remaining[self.max_ethereum_batch_size:]
                self.transfer_from_eth_to_furynode(eth_broker_account, fury_broker_account, batch, total_token_amount, 0)
                log.debug(f"Batch completed, {len(remaining)} tokens remaining")
            # Transfer ETH separately
            log.debug("Thansfering ETH from ethereum to furynode...")
            self.transfer_from_eth_to_furynode(eth_broker_account, fury_broker_account, [], 0, total_eth_amount_gwei)
        else:
            log.debug(f"Transferring {len(tokens_to_transfer)} tokens from ethereum to furynode in single batch...")
            self.transfer_from_eth_to_furynode(eth_broker_account, fury_broker_account, tokens_to_transfer, total_token_amount, total_eth_amount_gwei)
        self.distribute_tokens_to_wallets(fury_broker_account, tokens_to_transfer, token_amount, target_fury_accounts, eth_amount_gwei)
        ...
        log.info("To see newly minted tokens in UI, you need to edit 'scripts/ibc/tokenregistry/generate-erc20-jsons.sh' "
            ...
            "and commit the results in the furynet-devnet-1 folder. @tim will pick up the PR and register it on "
    ...
    def transfer_eth(self, from_eth_addr: eth.Address, amount_gwei: int, target_fury_accounts: Iterable[cosmos.Address]):
        pending_txs = []
        for fury_acct in target_fury_accounts:
            txrcpt = self.ctx.tx_bridge_bank_lock_eth(from_eth_addr, fury_acct, amount_gwei * eth.GWEI)
            pending_txs.append(txrcpt)
        self.wait_for_all(pending_txs)


def run(*args):
    # This script should be run with FURYTOOL_ENV_FILE set to a file containing definitions for OPERATOR_ADDRESS,
