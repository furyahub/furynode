...
from furytool import command, cosmos, project, environments, furynet
...
# Test scenarios (James): https://www.notion.so/furynet/Minimum-Commissions-Max-Voting-Power-Test-Scenarios-Draft-729620045e2d41f8b18f3a5df28b623b
# How to bypass the 24h limit (Caner): https://www.notion.so/furynet/v0-15-0-Edit-min-commission-of-existing-validator-edecd16b074a4900974704d223847b48
# Useful info:
# - https://app.zenhub.com/workspaces/current-sprint---engineering-615a2e9fe2abd5001befc7f9/issues/furynet/furynet-chainops/200
# Upgrades:
# - https://github.com/Furynet/furynet-devops/blob/main/scripts/furynode/release/testing/upgrade_path.json
# - https://github.com/Furynet/furynode/blob/68f69eb7e390363f336ec7a235ab7e564bf5dabb/scripts/upgrade-integration.sh#L39-L39
...
def create_environment(cmd, version, commission_rate=0.06, commission_max_rate=0.10, commission_max_change_rate=0.05,
    ...
    home_root = "/tmp/furytool.tmp/test_min_commission"
    cmd.rmdir(home_root)
    cmd.mkdir(home_root)
...
def test_min_commission_create_new_validator(cmd: command.Command):
    def test_case(commission_rate, commission_max_rate, should_succeed):
        ...
        if should_succeed:
            ...
            assert furynet.is_min_commission_too_low_exception(exception)
...
def test_max_voting_power(cmd: command.Command):
    def test_case(from_validator_index, to_validator_index, amount, should_succeed):
        ...
        if should_succeed:
            ...
            assert furynet.is_max_voting_power_limit_exceeded_exception(exception)
            assert validator_powers_after == validator_powers_before

    fury = 0
    akasha = 1

    test_case(akasha, fury, 100, False)  # Current (and projected) voting power too big
    test_case(fury, akasha, 100, True)
    test_case(fury, akasha, 10**23, False)  # Projected voting power too big
