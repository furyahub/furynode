...
class TestMargin:
    ...
    def with_test_env(self, pool_setup):
        ...
        margin_params_before = env.furynd.query_margin_params()
        env.furynd.tx_margin_update_pools(env.clp_admin, mtp_enabled_pools, [], broadcast_mode="block")
        margin_params_after = env.furynd.query_margin_params()
        assert len(margin_params_before["params"]["pools"]) == 0
        assert set(margin_params_after["params"]["pools"]) == mtp_enabled_pools

        yield env, env.furynd, pool_definitions
        env.close()

    def test_swap_fury_to_external(self):
        with self.with_test_env(self.default_pool_setup) as (env, furynd, pools_definitions):
            src_denom = "fury"
            dst_denom = "ceth"
            swap_amount = 10**18
            assert dst_denom in pools_definitions

            account = furynd.create_addr()
            env.fund(account, {FURY: 10**25 + swap_amount})
            balance_before = furynd.get_balance(account)
            pool_before = furynd.query_pools_sorted()[dst_denom]
            furynd.tx_clp_swap(account, src_denom, swap_amount, dst_denom, 0, broadcast_mode="block")
            balance_after = furynd.get_balance(account)
            pool_after = furynd.query_pools_sorted()[dst_denom]
    ...
    def test_swap_external_to_external(self):
        with self.with_test_env(self.default_pool_setup) as (env, furynd, pools_definitions):
            ...
            balances.append(furynd.get_balance(account))
            pools.append(furynd.query_pools_sorted())
            env.fund(account, {FURY: 10**20, src_denom: swap_amount})

            balance_before = furynd.get_balance(account)
            pools_before = furynd.query_pools_sorted()
            furynd.tx_clp_swap(account, src_denom, swap_amount, dst_denom, 0, broadcast_mode="block")
            balance_after = furynd.get_balance(account)
            pools_after = furynd.query_pools_sorted()
    ...
    def test_margin(self):
        ...
        with self.with_test_env(self.default_pool_setup) as (env, furynd, pools_definitions):
            ...
            margin_params = furynd.query_margin_params()

            # furynd.tx_margin_whitelist(env.clp_admin, account, broadcast_mode="block")

            pool_before_open = furynd.query_pools_sorted()[collateral_asset]
            balance_before_open = furynd.get_balance(account)
            mtp_positions_before_open = furynd.query_margin_positions_for_address(account)
            res = furynd.margin_open_simple(account, borrow_asset, collateral_asset=collateral_asset,
                collateral_amount=collateral_amount, leverage=leverage, position="long")
            mtp_id = int(res["id"])
            pool_after_open = furynd.query_pools_sorted()[collateral_asset]
            balance_after_open = furynd.get_balance(account)
            mtp_positions_after_open = furynd.query_margin_positions_for_address(account)
            ...
            for i in range(10):
                cnt = len(furynd.query_margin_positions_for_address(account))
                if cnt == 0:
                    break
            ...
            pool_before_close = furynd.query_pools_sorted()[collateral_asset]
            balance_before_close = furynd.get_balance(account)
            res2 = furynd.tx_margin_close(account, mtp_id, broadcast_mode="block")
            pool_after_close = furynd.query_pools_sorted()[collateral_asset]
            balance_after_close = furynd.get_balance(account)
