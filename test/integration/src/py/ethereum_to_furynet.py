...
if __name__ == "__main__":
    ...
    request = burn_lock_functions.EthereumToFurynetTransferRequest.from_args(args)

    logging.info(f"transferrequestjson: {json.dumps(request.__dict__)}")

    transfer_result = burn_lock_functions.transfer_ethereum_to_furynet(request, max_seconds=30)
    final_balance = transfer_result["furynet_ending_balance"]
