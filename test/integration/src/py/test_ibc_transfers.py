...
def test_burn_ibc_coins(
    ...
    request, credentials = generate_test_account(
        ...
        target_fury_balance=furynet_fees_int * 2 + small_amount
    )
    ...
    request.furynet_symbol = feedface_token
...
def test_lock_ibc_coins(
    ...
    feedface_transfer_request.furynet_address = fury_source
    feedface_transfer_request.furynet_destination_address = request.furynet_address
    feedface_transfer_request.furynet_symbol = feedface_token
    feedface_transfer_request.amount = 100

    burn_lock_functions.send_from_furynet_to_furynet(feedface_transfer_request, fury_source_integrationtest_env_credentials)
    ...
    request.furynet_symbol = feedface_token
    request.amount = small_amount
    burn_lock_functions.send_from_furynet_to_ethereum(request, credentials)
    ...
    logging.info("send FEEDFACE back to furynet")
    burn_lock_functions.transfer_ethereum_to_furynet(request, 10)
