...
class EthereumToFurynetTransferRequest:
    furynet_address: str = ""
    furynet_destination_address: str = ""
    ethereum_address: str = ""
    ethereum_private_key_env_var: str = "not required for localnet"
    furynet_symbol: str = "ceth"
    ethereum_symbol: str = "eth"
    ethereum_network: str = ""  # mainnet, ropsten, http:// for localnet
    amount: int = 0
    ceth_amount: int = 0
    furynet_fees: str = ""  # Deprecated, see https://github.com/Furynet/furynode/pull/1802#discussion_r697403408
    ...
    @staticmethod
    def from_args(args):
        return EthereumToFurynetTransferRequest(
            furynet_address=args.furynet_address[0],
            furynet_destination_address=args.furynet_destination_address[0],
            ethereum_address=args.ethereum_address[0],
            furynet_symbol=args.furynet_symbol[0],
...
def start_ebrelayer():
    ...
    return get_shell_output(f"{integration_dir}/furynet_start_ebrelayer.sh")
...
def get_eth_balance(transfer_request: EthereumToFurynetTransferRequest):
...
def get_whitelisted_tokens(transfer_request: EthereumToFurynetTransferRequest):
    network_element = f"--ethereum_network {transfer_request.ethereum_network} " if transfer_request.ethereum_network else ""
    symbol_element = f"--symbol {transfer_request.ethereum_symbol} " if transfer_request.ethereum_symbol else ""
    private_element = f"--ethereum_private_key_env_var \"{transfer_request.ethereum_private_key_env_var}\"" if transfer_request.ethereum_private_key_env_var else ""
    command_line = " ".join(
...
def mint_tokens(transfer_request: EthereumToFurynetTransferRequest, operator_address):
    network_element = f"--ethereum_network {transfer_request.ethereum_network} " if transfer_request.ethereum_network else ""
    symbol_element = f"--symbol {transfer_request.ethereum_symbol} " if transfer_request.ethereum_symbol else ""
    private_element = f"--ethereum_private_key_env_var \"{transfer_request.ethereum_private_key_env_var}\"" if transfer_request.ethereum_private_key_env_var else ""
    command_line = " ".join(
...
def wait_for_eth_balance(transfer_request: EthereumToFurynetTransferRequest, target_balance, max_seconds=80):
    wait_for_balance(
...
def wait_for_furynet_addr_balance(
        furynet_address,
        symbol,
        target_balance,
        furynetcli_node,
    ...
    if not max_seconds:
        max_seconds = 90
    logging.debug(f"wait_for_furynet_addr_balance for node {furynetcli_node}, {normalized_symbol}, {target_balance}")
    return wait_for_balance(
        lambda: int(get_furynet_addr_balance(furynet_address, furynetcli_node, normalized_symbol)),
        int(target_balance),
        max_seconds,
        debug_prefix
    )
...
def send_from_furynet_to_furynet_cmd(
        transfer_request: EthereumToFurynetTransferRequest,
        credentials: FurynetcliCredentials
):
    logging.debug(f"send_from_furynet_to_furynet {transfer_request} {credentials}")
    ...
    cmd = " ".join([
        ...
        chain_id_entry,
        node,
        f"{transfer_request.amount}{transfer_request.furynet_symbol}",
        furynet_fees_entry,
        home_entry,
        "--gas auto",
        "-y -o json",
...
def send_from_furynet_to_furynet(
        transfer_request: EthereumToFurynetTransferRequest,
        credentials: FurynetcliCredentials
):
...
def send_from_furynet_to_ethereum_cmd(
        transfer_request: EthereumToFurynetTransferRequest,
        credentials: FurynetcliCredentials,
):
    """
    Sends from Furynet to Ethereum.
    ...
    command_line = f"{yes_entry} " \
                   ...
                   f"{transfer_request.furynet_symbol} " \
                   f"{ceth_charge} " \
                   f"{keyring_backend_entry} " \
                   f"{furynet_fees_entry} " \
...
def send_from_furynet_to_ethereum(transfer_request: EthereumToFurynetTransferRequest,
                                   credentials: FurynetcliCredentials):
...
def send_from_ethereum_to_furynet(transfer_request: EthereumToFurynetTransferRequest) -> int:
    direction = "sendBurnTx" if transfer_request.furynet_symbol == "fury" else "sendLockTx"
    command_line = f"yarn -s --cwd {transfer_request.smart_contracts_dir} integrationtest:{direction} " \
                   f"--furynet_address {transfer_request.furynet_address} " \
...
def wait_for_fury_account(fury_addr, furynetcli_node, max_seconds=90):
    def fn():
        try:
            get_furynet_addr_balance(fury_addr, furynetcli_node, "eth")
            return True
        except:
            return False

    wait_for_predicate(lambda: fn(), True, max_seconds, f"wait for account {fury_addr}")
...
def wait_for_ethereum_block_number(block_number: int, transfer_request: EthereumToFurynetTransferRequest):
    network_element = f"--ethereum_network {transfer_request.ethereum_network} " if transfer_request.ethereum_network else ""
...
def approve_token_amount(token_request: EthereumToFurynetTransferRequest):
...
def furynet_symbol_to_ethereum_symbol(s: str):
    if s == "fury":
        return "efury"
    elif s == "ceth":
        return NULL_ADDRESS
