...
from test_utilities import get_shell_output, get_furynet_addr_balance, \
    advance_n_ethereum_blocks, n_wait_blocks, \
    send_from_ethereum_to_furynet
from test_utilities import wait_for_furynet_addr_balance, \
    get_required_env_var, \
    EthereumToFurynetTransferRequest
...
def test_rollback_chain(source_ethereum_address, solidity_json_path):
    ...
    new_account = burn_lock_functions.create_new_furyaddr(credentials=credentials, keyname=new_account_key)
    ...
    request = EthereumToFurynetTransferRequest(
        furynet_address=new_account["address"],
    ...
    burn_lock_functions.transfer_ethereum_to_furynet(request, 50)
    ...
    initial_user_balance = get_furynet_addr_balance(new_addr, "", request.furynet_symbol)
    logging.info(f"initial_user_balance {initial_user_balance}")

    transfer_1 = send_from_ethereum_to_furynet(transfer_request=request)
    ...
    # the transaction should not have happened on the furynet side yet
    ...
    second_user_balance = get_furynet_addr_balance(new_addr, "", request.furynet_symbol)
    ...
    balance_after_sleep = get_furynet_addr_balance(new_addr, "", request.furynet_symbol)
    logging.info(f"get_furynet_addr_balance after sleep is {balance_after_sleep} for {new_addr}")

    expected_balance = initial_user_balance + request.amount
    logging.info(f"look for a balance of {expected_balance}")
    wait_for_furynet_addr_balance(new_addr, request.furynet_symbol, expected_balance, "")
