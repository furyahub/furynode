...
from furytool import eth, furynet
from furytool.inflate_tokens import InflateTokens
from furytool.common import *


# Furynet wallets to which we want to distribute
test_wallets = [
    "fury1fpq67nw66thzmf2a5ng64cd8p8nxa5vl9d3cm4",
    "fury1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
    "fury1hjkgsq0wcmwdh8pr3snhswx5xyy4zpgs833akh",
    "fury1ypc5qcq5ha562xlak4xw3g6v352k39t6868jhx",
    "fury1u7cp5e5kty8xwuu7k234ah4jsknvkzazqagvl6",
    "fury1lj3rsayj4xtrhp2e3elv4nf7lazxty272zqegr",
    "fury1cffgyxgvw80rr6n9pcwpzrm6v8cd6dax8x32f5",
    "fury1dlse3w2pxlmuvsj5eda344zp99fegual958qyr",
    "fury1m7257566ehx7ya4ypeq7lj4y2h075z6u2xu79v",
    "fury1qrxylp97p25wcqn4cs9nd02v672073ynpkt4yr",
    "fury13rysrrdlhtmuc2pzve7jk0t4pwytwyxhaqcqcn",
    "fury1shywxv2g8gvjcqknvkxu4p6lkqhfclwwj2qk6h",
    "fury1gqm44p5ax4kgk6hksxgv4vuh2adue2acxvg542",
    "fury1zwgc9frcfpt3hhkqfu9u7up94ag5rp30kwrwrj",
]
...
def _test_inflate_tokens_parametrized(ctx, number_of_tokens):
    ...
    balances_before = [ctx.get_furynet_balance(w) for w in wallets]
    script.transfer(requested_tokens, amount_in_tokens, wallets, amount_gwei)
    balances_delta = [furynet.balance_delta(balances_before[i], ctx.get_furynet_balance(w)) for i, w in enumerate(wallets)]

    for balances_delta in balances_delta:
        for t in requested_tokens:
            assert balances_delta[ctx.eth_symbol_to_fury_symbol(t["symbol"])] == amount_in_tokens * 10**t["decimals"]
