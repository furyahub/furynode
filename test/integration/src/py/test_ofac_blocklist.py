...
from furytool import eth, test_utils
...
def bridge_bank_lock_eth(ctx, from_eth_acct, to_fury_acct, amount):
    assert ctx.eth.get_eth_balance(from_eth_acct) > max_gas_required * max_gas_price, "Not enough gas for test"
    return ctx.bridge_bank_lock_eth(from_eth_acct, to_fury_acct, amount)

def bridge_bank_lock_erc20(ctx, bridge_token, from_eth_acct, to_fury_acct, amount):
    assert ctx.eth.get_eth_balance(from_eth_acct) > max_gas_required * max_gas_price, "Not enough gas for test"
    assert ctx.get_erc20_token_balance(bridge_token.address, from_eth_acct) >= amount, "Not enough tokens for test"
    return ctx.bridge_bank_lock_erc20(bridge_token, from_eth_acct, to_fury_acct, amount)
...
def test_blocklist_eth(ctx):
    ...
    to_fury_acct = ctx.create_furynet_addr()
    fury_symbol = test_utils.CETH
    ...
    def assert_blocked(addr):
        ...
        try:
            bridge_bank_lock_eth(ctx, addr, to_fury_acct, amount_to_send)
            assert False
    ...
    # Valid positive test outcome: event emitted, optionally: funds appear on furynet
    def assert_not_blocked(addr):
        assert len(filter.get_new_entries()) == 0

        balances_before = ctx.get_furynet_balance(to_fury_acct)
        txrcpt = bridge_bank_lock_eth(ctx, addr, to_fury_acct, amount_to_send)
        ctx.advance_blocks()
        balances_after = ctx.furynode.wait_for_balance_change(to_fury_acct, balances_before)

        assert balances_after.get(fury_symbol, 0) == balances_before.get(fury_symbol, 0) + amount_to_send
        ...
        assert entries[0].args["_to"] == test_utils.fury_addr_to_evm_arg(to_fury_acct)
...
def test_blocklist_erc20(ctx):
    ...
    fury_token_symbol = ctx.eth_symbol_to_fury_symbol(eth_token_symbol)
    ...
    def assert_blocked(addr):
        ...
        try:
            bridge_bank_lock_erc20(ctx, test_token, addr, to_fury_acct, amount_to_send)
            assert False
        except Exception as e:
            assert is_blocklisted_exception(ctx, e)
    ...
    def assert_not_blocked(addr):
        ...
        balances_before = ctx.get_furynet_balance(to_fury_acct)
        txrcpt = bridge_bank_lock_erc20(ctx, test_token, addr, to_fury_acct, amount_to_send)
        ctx.advance_blocks()
        balances_after = ctx.furynode.wait_for_balance_change(to_fury_acct, balances_before)

        assert balances_after.get(fury_token_symbol, 0) == balances_before.get(fury_token_symbol, 0) + amount_to_send
