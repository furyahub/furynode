...
def test_bulk_transfers(
    ...
    new_addresses_and_keys = list(map(lambda x: create_new_furyaddr_and_key(), range(n_transfers)))
    ...
    for furyaddr, ethaddr in zip(new_addresses_and_keys, new_eth_addrs):
        ...
        r.furynet_address = furyaddr[0]
        r.ethereum_address = ethaddr["address"]
        r.amount = 100
        simple_credentials.from_key = furyaddr[1]
