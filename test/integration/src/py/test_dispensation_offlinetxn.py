...
import string
import random
from dispensation_envutils import create_offline_singlekey_txn, create_new_furyaddr_and_key, send_sample_fury, balance_check, \
...
def test_create_offline_singlekey_txn(claimType):
    ...
    assert str(msg_type) == '/furynode.dispensation.v1.MsgCreateDistribution'
...
def test_run_offline_singlekey_txn(claimType):
    ...
    assert str(run_msg_type) == '/furynode.dispensation.v1.MsgRunDistribution'
