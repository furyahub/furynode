...
from test_utilities import get_required_env_var, get_shell_output, FurynetcliCredentials
...
def test_transfer_eth_to_ceth_using_replay_blocks(
    ...
    fury_transfer_request.furynet_symbol = "fury"
    small_amount = 9
    fury_transfer_request.amount = small_amount
    test_utilities.send_from_furynet_to_ethereum(fury_transfer_request, credentials)
    ...
    logging.info(f"send {small_amount} fury to {request.furynet_address}")
    test_utilities.send_from_ethereum_to_furynet(request)
    ...
    time.sleep(5)
    balance_with_no_relayer = test_utilities.get_furynet_addr_balance(
        request.furynet_address, request.furynoded_node,
        request.furynet_symbol
    )
 ...
 --keyring-backend test --node tcp://0.0.0.0:26657 --from {mon}  --symbol-translator-file {integration_dir}/config/symbol_translator.json"""
    ...
    time.sleep(5)
    logging.info(f"check the ending balance of {request.furynet_address} after replaying blocks")
    ending_balance = test_utilities.get_furynet_addr_balance(request.furynet_address, request.furynoded_node,
                                                              request.furynet_symbol)
    ...
    test_utilities.get_shell_output(cmd)
    time.sleep(5)
    ending_balance2 = test_utilities.get_furynet_addr_balance(request.furynet_address, request.furynoded_node,
                                                               request.furynet_symbol)
