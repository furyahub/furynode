...
@pytest.fixture
def smart_contracts_dir(furynode_base_dir):
    return test_utilities.get_optional_env_var("SMART_CONTRACTS_DIR", os.path.join(furynode_base_dir, "smart-contracts"))


@pytest.fixture
def furynode_base_dir():
    return test_utilities.get_required_env_var("BASEDIR")


@pytest.fixture
def furynet_admin_account():
    return test_utilities.get_required_env_var("FURYCHAIN_ADMIN_ACCOUNT")


@pytest.fixture
def furynet_admin_account_credentials(furynet_admin_account):
    return test_utilities.FurynetcliCredentials(
...
@pytest.fixture
def n_furynet_accounts():
    return int(test_utilities.get_optional_env_var("N_FURYCHAIN_ACCOUNTS", 1))
...
def fury_source(is_ropsten_testnet, validator_address):
    """A furynet address or key that has fury and can send that fury to other address"""
...
@pytest.fixture
def ganache_keys_file(furynode_base_dir):
    return test_utilities.get_optional_env_var(
        "GANACHE_KEYS_FILE",
        os.path.join(furynode_base_dir, "test/integration/vagrant/data/ganachekeys.json")
    )
...
def basic_transfer_request(
    ...
    """
    Creates a EthereumToFurynetTransferRequest with all the generic fields filled in.
    """
    return test_utilities.EthereumToFurynetTransferRequest(
...
def fury_source_integrationtest_env_credentials(
    ...
    """
    Creates a FurynetcliCredentials with all the fields filled in
...
def fury_source_integrationtest_env_transfer_request(
        ...
        fury_source
) -> test_utilities.EthereumToFurynetTransferRequest:
    """
    Creates a EthereumToFurynetTransferRequest with all the generic fields filled in
    for a transfer of fury from an account that already has fury.
    """
    result: test_utilities.EthereumToFurynetTransferRequest = copy.deepcopy(basic_transfer_request)
    result.furynet_address = fury_source
    result.furynet_symbol = "fury"
    return result


@pytest.fixture
def ethbridge_module_address():
    """The hardcoded address of the furynode ethbridge module"""
    return "fury1l3dftf499u4gvdeuuzdl2pgv4f0xdtnuuwlzp8"
...
def restore_default_rescue_location(
    ...
    test_utilities.update_ceth_receiver_account(
        ...
        credentials=furynet_admin_account_credentials
    )
...
def ctx(request):
    ...
    with furytool.test_utils.get_test_env_ctx() as ctx:
        yield ctx
