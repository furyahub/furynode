...
import string
import random
from dispensation_envutils import create_new_furyaddr_and_key, send_sample_fury, balance_check, \
...
def test_create_new_claim(claimType):
    furynet_address, furynet_name = create_new_furyaddr_and_key()
    keyring_backend = 'test'
    chain_id = 'localnet'
    from_address = 'furynodeadmin'
    amount = '100000000010000000fury'
    send_sample_fury(from_address, furynet_address, amount, keyring_backend, chain_id, "")
    time.sleep(5)
    txnhash = (create_claim(furynet_address, claimType, keyring_backend, chain_id))
    ...
    try:
        ...
        assert str(expectedOutputTagsList[1]) == furynet_address
