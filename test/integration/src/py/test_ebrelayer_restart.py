...
from pytest_utilities import create_new_furyaddr
from pytest_utilities import generate_test_account, generate_minimal_test_account
from test_utilities import EthereumToFurynetTransferRequest
...
def test_ebrelayer_restart(
    ...
    balance = test_utilities.get_furynet_addr_balance(request.furynet_address, request.furynoded_node, "ceth")
    ...
    time.sleep(5)
    assert balance == test_utilities.get_furynet_addr_balance(request.furynet_address, request.furynoded_node,
                                                               "ceth")
...
def test_ethereum_transactions_with_offline_relayer(
    ...
    amount = 9000
    new_addresses = list(map(lambda x: create_new_furyaddr(), range(3)))
    ...
    yarn_result = test_utilities.run_yarn_command(
        " ".join([
            ...
            f"--furynet_address {new_addresses[0]}",
    ...
    for a in new_addresses:
        ...
        test_utilities.wait_for_furynet_addr_balance(a, "ceth", amount, basic_transfer_request.furynoded_node, 90)


@pytest.mark.usefixtures("ensure_relayer_restart")
def test_furynet_transactions_with_offline_relayer(
        basic_transfer_request: EthereumToFurynetTransferRequest,
        fury_source_integrationtest_env_credentials: test_utilities.FurynetcliCredentials,
    ...
    for a in new_eth_addrs:
        ...
        furynet_balance = test_utilities.get_furynet_addr_balance(request.furynet_address, request.furynoded_node,
                                                                    "ceth")
        logging.info(f"furynet balance is {furynet_balance}, request is {request}")
        test_utilities.send_from_furynet_to_ethereum(
