...
import (
	...
	furyAnte "github.com/Furynet/furynode/app/ante"
...
func TestAnte_CalculateDelegateProjectedVotingPower(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			vcd := furyAnte.NewValidateMinCommissionDecorator(app.StakingKeeper, app.BankKeeper)

			delegatedTokens := tc.otherValidatorBondedTokens.Add(tc.otherValidatorUnbondedTokens.Add(tc.validatorTokens))
			delegatorAddresses := furyapp.AddTestAddrs(app, ctx, 1, delegatedTokens)
			delegatorAddress := delegatorAddresses[0]
			pubKeys := furyapp.CreateTestPubKeys(3)
...
func createValidator(app *furyapp.FurynetApp, ctx sdk.Context, delegateAmount sdk.Int, status stakingtypes.BondStatus, delegatorAddress sdk.AccAddress, valPubKey cryptotypes.PubKey) sdk.ValAddress {
