...
* Two furynode instances running independent chains.
...
2. Build a new `furynode` image:

```bash
SERVICE=furynode make build-image
```
...
|Var|Description|
|---|-----------|
|`${CHAINNET0}`|The Chain ID of the first furynode (e.g.: `furynet-ibc-0`)|
|`${CHAINNET1}`|The Chain ID of the second furynode (e.g.: `furynet-ibc-1`)|
|`${IPADDR0}`|The IP Address of the first furynode (e.g.: `192.168.65.2`)|
|`${IPADDR1}`|The IP Address of the second furynode (e.g.: `192.168.65.3`)|
...
|`${MNEMONIC}`|The mnemonic both furynode's will use for their genesis accounts.|
