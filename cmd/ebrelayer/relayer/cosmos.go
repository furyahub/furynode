...
import (
	...
	"github.com/Furynet/furynode/cmd/ebrelayer/internal/symbol_translator"

	"github.com/Furynet/furynode/cmd/ebrelayer/contract"
	cosmosbridge "github.com/Furynet/furynode/cmd/ebrelayer/contract/generated/bindings/cosmosbridge"
	"github.com/Furynet/furynode/cmd/ebrelayer/txs"
	"github.com/Furynet/furynode/cmd/ebrelayer/types"
...
func (sub CosmosSub) Start(completionEvent *sync.WaitGroup, symbolTranslator *symbol_translator.SymbolTranslator) {
	...
	if err != nil {
		sub.SugaredLogger.Errorw("failed to initialize a furynet client.",
	...
	if err := client.Start(); err != nil {
		sub.SugaredLogger.Errorw("failed to start a furynet client.",
			errorMessageKey, err.Error())
		completionEvent.Add(1)
		go sub.Start(completionEvent, symbolTranslator)
		return
	...
	if err != nil {
		sub.SugaredLogger.Errorw("furynet client failed to subscribe to query.",
			errorMessageKey, err.Error(),
			"query", query)
		completionEvent.Add(1)
		go sub.Start(completionEvent, symbolTranslator)
	...
	defer func() {
		if err := client.Unsubscribe(context.Background(), "test", query); err != nil {
			sub.SugaredLogger.Errorw("furynet client failed to unsubscribe query.",
				errorMessageKey, err.Error())
		}
	}()
	...
	for {
		...
		case e := <-results:
			...
			if !ok {
				sub.SugaredLogger.Errorw("furynet client failed to extract event data from new block.",
			...
			sub.SugaredLogger.Infow("new furynet block witnessed")
			...
			for blockNumber := startBlockHeight; blockNumber <= blockHeight; {
				...
				if err != nil {
					sub.SugaredLogger.Errorw("furynet client failed to get a block.",
						errorMessageKey, err.Error())
					continue
				}
				...
				for _, txLog := range block.TxsResults {
					for _, event := range txLog.Events {
						...
						case types.MsgBurn, types.MsgLock:
							...
							if err != nil {
								sub.SugaredLogger.Errorw("furynet client failed in get message from event.",
									errorMessageKey, err.Error())
								continue
							}

							sub.SugaredLogger.Infow(
								"Received message from furynet: ",
								"msg", cosmosMsg,
							)
...
func MyDecode(data []byte) (types.ProphecyClaimUnique, error) {
	...
	// the length of furynode acc account is 42
