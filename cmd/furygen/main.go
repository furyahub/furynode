...
import (
	...
	"github.com/Furynet/furynode/tools/furygen"
	"github.com/spf13/cobra"
)

func main() {
	rootCmd := &cobra.Command{Use: "furygen"}
...
func networkCreateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			network := furygen.NewFurygen(&args[0]).NewNetwork(keyringBackend)
...
func networkResetCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			furygen.NewFurygen(&args[0]).NetworkReset(args[1])
		},
	}
}
...
func nodeCreateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			node := furygen.NewFurygen(&args[0]).NewNode(keyringBackend)
			node.Moniker = args[1]
...
func nodeResetStateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 1 {
				furygen.NewFurygen(&args[0]).NodeReset(nil)
			} else {
				furygen.NewFurygen(&args[0]).NodeReset(&args[1])
			}
		},
	}
}
...
func keyGenerateMnemonicCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.MaximumNArgs(0),
		Run: func(cmd *cobra.Command, args []string) {
			furygen.NewFurygen(nil).KeyGenerateMnemonic("", "")
		},
	}
}
...
func keyRecoverFromMnemonicCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.MinimumNArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			furygen.NewFurygen(nil).KeyRecoverFromMnemonic(args[0])
		},
	}
}
