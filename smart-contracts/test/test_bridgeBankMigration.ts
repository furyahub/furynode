...
import {FurynetContractFactories} from "../src/tsyringe/contracts";
...
import {
    getProxyAdmin,
    impersonateAccount,
    setupFurynetMainnetDeployment,
    startImpersonateAccount
} from "../src/hardhatFunctions"
import {FurynetAccountsPromise} from "../src/tsyringe/furynetAccounts";
...
describe("BridgeBank and CosmosBridge - updating to latest smart contracts", () => {
    ...
    before('use mainnet data', async () => {
        await setupFurynetMainnetDeployment(container, hardhat)
    })
    ...
    describe("upgraded BridgeBank", async () => {
        it("should maintain existing stored values", async () => {
            ...
            const bridgeBankFactory = await container.resolve(FurynetContractFactories).bridgeBank
        ...
        it("should lock and burn via existing validators", async () => {
            ...
            await impersonateAccount(hardhat, upgradeAdmin, hardhat.ethers.utils.parseEther("10"), async fakeDeployer => {
                ...
                const accounts = await container.resolve(FurynetAccountsPromise).accounts
                const bridgeBankFactory = await container.resolve(FurynetContractFactories).bridgeBank
                ...
                const cosmosBridgeFactory = (await container.resolve(FurynetContractFactories).cosmosBridge).connect(fakeDeployer)
                ...
                const testTokenFactory = (await container.resolve(FurynetContractFactories).bridgeToken).connect(operator)
                ...
                }
                {
                    ...
                    const recipient = web3.utils.utf8ToHex("fury1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace")
...
async function executeNewProphecyClaimWithTestValues(
    ...
    const cosmosSender = web3.utils.utf8ToHex("fury1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace")
