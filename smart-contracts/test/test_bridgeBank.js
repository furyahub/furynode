...
contract("BridgeBank", function (accounts) {
  ...
  describe("Bridge token minting (for burned Cosmos assets)", function () {
    beforeEach(async function () {
      ...
      this.sender = web3.utils.utf8ToHex(
        "fury1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace"
      );
    ...
    it("should return true if a furynet address prefix is correct", async function () {
      (await this.bridgeBank.verifyFuryPrefix(this.sender)).should.be.equal(true);
    })

    it("should return false if a furynet address has an incorrect `fury` prefix", async function () {
      const incorrectFuryAddress = web3.utils.utf8ToHex(
        "eif1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace"
      );
      (await this.bridgeBank.verifyFuryPrefix(incorrectFuryAddress)).should.be.equal(false);
    })
  ...
  describe("Bridge token deposit locking (Ethereum/ERC20 assets)", function () {
    ...
    it("should not allow users to lock ERC20 tokens if the furyaddress length is incorrect", async function () {
      const invalidFuryAddress = web3.utils.utf8ToHex(
    ...
    it("should not allow users to lock ERC20 tokens if the furyaddress prefix is incorrect", async function () {
      const invalidFuryAddress = web3.utils.utf8ToHex(
  ...
  // cosmos assets on furynet, and then we hook into an existing ERC20 contract on mainnet
  ...
  describe("Bridge token creation", function () {
    ...
    it("should burn eFury to create fury on furynet", async function () {
      ...
      const symbol = 'eFury'
      const amount = 100000;
      const furyAddress = "0x" + convertToHex("fury12qfvgsq76eghlagyfcfyt9md2s9nunsn40zu2h");
    ...
    it("should NOT burn eFury to create fury on furynet if user is blocklisted", async function () {
      // Add sender to the blocklist
    ...
    it("should mint eFury to transfer Fury from furynet to ethereum", async function () {
      function convertToHex(str) {
        let hex = '';
        for (let i = 0; i < str.length; i++) {
            hex += '' + str.charCodeAt(i).toString(16);
      ...
      const cosmosSender = "0x" + convertToHex("fury12qfvgsq76eghlagyfcfyt9md2s9nunsn40zu2h");
