...
import {
    ...
    BridgeTokenSetup,
    FuryContract,
    FurynetContractFactories
} from "../src/tsyringe/contracts";
import {BridgeBank, BridgeBank__factory, BridgeToken} from "../build";
import {FurynetAccounts, FurynetAccountsPromise} from "../src/tsyringe/furynetAccounts";
import {
    DeploymentChainId,
    DeploymentDirectory,
    DeploymentName,
...
import {impersonateAccount, setupFurynetMainnetDeployment} from "../src/hardhatFunctions";
...
describe("BridgeBank", () => {
    ...
    describe("locking and burning", function () {
        ...
        const invalidRecipient = web3.utils.utf8ToHex("efury1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace")

        before('create test token', async () => {
            accounts = await container.resolve(FurynetAccountsPromise).accounts
            sender = accounts.availableAccounts[0]
            amount = hardhat.ethers.utils.parseEther("100") as BigNumber
            smallAmount = amount.div(100)
            const testTokenFactory = (await container.resolve(FurynetContractFactories).bridgeToken).connect(sender)
        ...
        it("should read the whitelist", async () => {
            const btf = await container.resolve(FurynetContractFactories).bridgeToken
