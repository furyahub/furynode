...
const DEPLOYMENT_NAME = process.env.DEPLOYMENT_NAME || "furynet-1";
...
const state = {
  addresses: {
    ...
    furyRecipient: web3.utils.utf8ToHex("fury1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace"),
...
async function lock({ expectedError }) {
  ...
  try {
    await state.contracts.upgradedBridgeBank
      .connect(state.signers.user1)
      .lock(state.addresses.furyRecipient, state.contracts.bridgeToken.address, state.amount, {
        value: 0,
      });
  } catch (e) {
...
function copyManifest() {
  ...
  fs.copySync(
    `./deployments/furynet-1/.openzeppelin/mainnet.json`,
