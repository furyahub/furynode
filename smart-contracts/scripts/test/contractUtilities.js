...
function buildProvider(context, argv, logging) {
    ...
    const {getRequiredEnvironmentVariable} = context.require('./furynetUtilities');
...
async function setAllowance(context, coinDenom, amount, argv, logging, requestParameters) {
    const furynetUtilities = context.require('./furynetUtilities');

    if (coinDenom != furynetUtilities.NULL_ADDRESS) {
        ...
        if (new BN(currentAllowance).lt(new BN(amount))) {
            const approveResult = await newToken.approve(argv.bridgebank_address, furynetUtilities.SOLIDITY_MAX_INT, requestParameters);
