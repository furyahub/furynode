...
module.exports = async (cb) => {
    ...
    const argv = furynetUtilities.processArgs(this, {
        ...
        'transactions': {
            type: "string",
            demandOption: true,
            description: 'json containing all the transactions to send.  Specify [{amount:, symbol:, furynet_address:}].  The entries in --amount, --symbol, --furynet_address are only used to estimate gas'
        },
        'lock_or_burn': {
            type: "string",
            default: "lock",
    ...
    for (const t of transactions) {
        logging.info(`calling bridgeBankContract.lock for ${t.furynet_address}, amount is |${t.amount}|`);
        let lockResult;
        const amount = new BN(t.amount);
        try {
            if (argv.lock_or_burn === "lock")
                lockResult = bridgeBankContract.lock(Web3.utils.utf8ToHex(t.furynet_address), t.symbol, amount, request);
            else
                lockResult = bridgeBankContract.burn(Web3.utils.utf8ToHex(t.furynet_address), t.symbol, amount, request);
        } catch (error){
    ...
    for (const a of actions) {
        ...
        logging.info(`bridgeBankContract.lock result for ${a.t.furynet_address}: ${JSON.stringify(result)}`);
        results.push(result);
