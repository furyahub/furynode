...
const furynodeDestinationFile = generateTodayFilename({
  directory: "data",
  prefix: "furynode_mainnet_update",
  extension: "json",
});
...
async function main() {
  ...
  for (let i = 0; i < addressList.length; i++) {
    try {
      ...
      // Now, the furynode side:
      const furynodeObj = { ...FURYNODE_MODEL };
      const v1Denom = generateV1Denom(symbol);
      furynodeObj.decimals = decimals;
      furynodeObj.base_denom = v1Denom;
      furynodeObj.denom = v1Denom;
      furynodeList.push(furynodeObj);
  ...
  fs.writeFileSync(
    furynodeDestinationFile,
    JSON.stringify(furynodeList, null, 2)
  );
  ...
  print(
    "magenta",
    `Furynode results have been written to ${furynodeDestinationFile}.`
  );
  print(
    "yellow",
