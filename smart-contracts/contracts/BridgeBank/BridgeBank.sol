    ...
    Pausable
{
    ...
    /*
     * @dev: Modifier to only allow valid fury addresses
     */
    modifier validFuryAddress(bytes memory _furyAddress) {
        require(_furyAddress.length == 42, "Invalid len");
        require(verifyFuryPrefix(_furyAddress) == true, "Invalid fury address");
        _;
    }
    ...
    /*
     * @dev: function to validate if a fury address has a correct prefix
     */
    function verifyFuryPrefix(bytes memory _furyAddress)
        public
        pure
        returns (bool)
    {
        bytes3 furyInHex = 0x736966;

        for (uint256 i = 0; i < furyInHex.length; i++) {
            if (furyInHex[i] != _furyAddress[i]) {
                return false;
            }
        }
        return true;
