...
in the validator set (i.e. those validators which vote on blocks)) - see https://github.com/cosmos/cosmos-sdk/blob/d0043914ba7c37c3a0d7039d2c2a2aca6b38a93b/x/staking/types/validator.go#L350-L356 and https://www.mintscan.io/furynet/validators - the cumulative share of the validators in
...
1. Try to delegate 100 tokens to `fury_val`. This would give `fury_val` a projected voting power
...
1. Try to redelegate 100 tokens from `akasha_val` to `to fury_val`. This would give `fury_val` a projected voting power of 
...
1. Try to redelegate 100000000000000000000000 tokens to `akasha_val` from `fury_val`. This would give `akasha_val` a projected voting power
