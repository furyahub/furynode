...
furynd tx dispensation claim ValidatorSubsidy --from akasha --keyring-backend test --yes --chain-id localnet -o json
furynd tx dispensation claim ValidatorSubsidy --from fury --keyring-backend test --yes --chain-id localnet -o json
# create airdrop
# mkey = multisig key
...
sleep 8
furynd q dispensation claims-by-type ValidatorSubsidy -o json
sleep 8
furynd tx dispensation create ValidatorSubsidy output.json fury1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd --gas 200064128 --from fury1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd --keyring-backend test --fees 100000fury --yes --chain-id furynet-devnet-042 --node tcp://rpc-devnet-042.furynet.finance:80

sleep 8
furynd q dispensation distributions-all --chain-id localnet -o json
