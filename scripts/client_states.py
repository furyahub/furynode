...
while True:
  output = subprocess.check_output(['furynd', 'q', 'ibc', 'client', 'states', '--output', 'json', '--node', sys.argv[1], '--page', str(page)])
...
# get the block time
output = subprocess.check_output(["furynd", "q", "ibc", "connection", "connections", "--output", "json", "--node", sys.argv[1]])

connection = json.loads(output.decode('utf-8'))

current_block_number = connection['height']['revision_height']
output = subprocess.check_output(["furynd", "q", "block", current_block_number, "--node", sys.argv[1]])
...
for client_data in clients:
  ...
  output = subprocess.check_output(['furynd', 'q', 'ibc', 'client', 'consensus-state', client_id,  f'{revision_number}-{revision_height}', '--node', sys.argv[1], '--output', 'json'])
