...
Take a furynode binary and build a docker image that forms the basis of the [`ui-stack`](https://github.com/orgs/Furynet/packages/container/package/furynode%2Fui-stack) image that our frontend UI is based on. This is the primary way we keep our repositories in sync.
...
| Script                       | Description                                                                                                                  |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `yarn stack`                 | Launch stack from scratch building furynode.                                                                                  |
...
We need to have a different deployment cadence for `ui` code compared to our `furynode` or blockchain code. In order to do this we need to be able to run our frontend e2e and integration tests against background builds.

## Structure

### Docker registry

We tie our repos together using a docker image generated on this repo and consumed by the frontend repo. You can view the latest docker repository uploads here: https://github.com/orgs/Furynet/packages/container/package/furynode%2Fui-stack
...
docker pull ghcr.io/furynet/furynode/ui-stack:develop
```
...
1. Launch a single furynode, ethereum node, and ebrelayer instance
...
Here is a recipe for how to work with the `ui` scripts in `furynode`.

### I want to change the furynet genesis information within the ui-stack.
...
1. Create a new branch on the `furynode` repo branching off `develop`.
1. Now you can make your changes which will probably be in one of the folowing places:

   1. The furynet genesis code is located here: `./ui/chains/fury/launch.sh` here we specify genesis tokens and amounts. Set up your tokens and amounts in this file.
   1. The CLP pool genesis code is located here: `./ui/chains/fury/migrate.sh` here we specify the default pools that are created and included in the ui-stack image.
...
1. Test your changes
   ...
   1. Change these as you require, ensure you have no docker containers running on the ports required and test by running the `yarn stack` command in the `furynode` (**backend**) repo. This will take a few minutes as it needs to setup and migrate all the chains.
...
- Every merge to `furynode/develop` and `furynode/master` publishes an image based off those branches.
...
In this case the backend will merge their breaking change to develop. Our GHA will run building the stack based on the latest change. After this the furynet-ui repo will be checked out and its tests applied to the built ui-stack within the github action. If this fails then the backend team know they have broken frontend.
