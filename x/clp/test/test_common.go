...
import (
	...
	"github.com/Furynet/furynode/x/clp/types"
)
...
func CreateTestAppClp(isCheckTx bool) (sdk.Context, *furyapp.FurynetApp) {
	return CreateTestAppClpWithBlacklist(isCheckTx, []sdk.AccAddress{})
}

func CreateTestAppClpWithBlacklist(isCheckTx bool, blacklist []sdk.AccAddress) (sdk.Context, *furyapp.FurynetApp) {
	furyapp.SetConfig(false)
	app := furyapp.SetupWithBlacklist(isCheckTx, blacklist)
	ctx := app.BaseApp.NewContext(isCheckTx, tmproto.Header{})
...
func CreateTestAppClpFromGenesis(isCheckTx bool, genesisTransformer func(*furyapp.FurynetApp, furyapp.GenesisState) furyapp.GenesisState) (sdk.Context, *furyapp.FurynetApp) {
	furyapp.SetConfig(false)
...
func GenerateRandomLP(numberOfLp int) []types.LiquidityProvider {
	...
	for i := 0; i < numberOfLp; i++ {
		externalToken := tokens[rand.Intn(len(tokens))]
		asset := types.NewAsset(TrimFirstRune(externalToken))
		lpAddess, err := sdk.AccAddressFromBech32("fury1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v")
		if err != nil {
			panic(err)
		}
...
func GeneratePoolsFromFile(app *furyapp.FurynetApp, keeper clpkeeper.Keeper, ctx sdk.Context) []*types.Pool {
