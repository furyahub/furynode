...
import (
	...
	admintest "github.com/Furynet/furynode/x/admin/test"
...
func TestMsgServer_DecommissionPool(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			poolUnits:           sdk.NewUint(1000),
			msg: &types.MsgDecommissionPool{
				Signer: "fury1jv65s3grqf6v6jl3dp4t6c9t9rk99cd8zzt2x5",
				Symbol: "eth",
			},
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			ctx, app := test.CreateTestAppClpFromGenesis(false, func(app *furyapp.FurynetApp, genesisState furyapp.GenesisState) furyapp.GenesisState {
...
func TestMsgServer_Swap(t *testing.T) {
	testcases := []struct {
	...
	}{
		{
			...
			errString: errors.New("Token not supported by furynet"),
		},
		{
...
func TestMsgServer_AddProviderDistribution(t *testing.T) {
	admin := "fury1gy2ne7m62uer4h5s4e7xlfq7aeem5zpwx6nu9q"
	nonAdmin := "fury1gy2ne7m62uer4h5s4e7xlfq7aeem5zpwx6nu9r"
	ctx, app := test.CreateTestAppClpFromGenesis(false, func(app *furyapp.FurynetApp, genesisState furyapp.GenesisState) furyapp.GenesisState {
