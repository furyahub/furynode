...
import (
	...
	"github.com/Furynet/furynode/x/oracle/types"
)
...
func (k Keeper) EnsureAddressIsInWhitelist(ctx sdk.Context, validatorAddress string) error {
	...
	k.Logger(ctx).Error(
		"furynode oracle keeper ProcessClaim validator not in whitelist",
...
func (k Keeper) ProcessClaim(ctx sdk.Context, claim types.Claim) (types.Status, error) {
	...
	if !activeValidator {
		logger.Error("furynode oracle keeper ProcessClaim validator not active.")
		return types.Status{}, types.ErrInvalidValidator
	}
	if claim.Id == "" {
		logger.Error("furynode oracle keeper ProcessClaim wrong claim id.", "claimID", claim.Id)
		return types.Status{}, types.ErrInvalidIdentifier
	}
	if claim.Content == "" {
		logger.Error("furynode oracle keeper ProcessClaim claim content is empty.")
