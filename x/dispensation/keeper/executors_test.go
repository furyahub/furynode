...
import (
	...
	"github.com/Furynet/furynode/x/dispensation/utils"
...
func createInput(t *testing.T, filename string) {
	in, err := sdk.AccAddressFromBech32("fury1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd")
	assert.NoError(t, err)
	out, err := sdk.AccAddressFromBech32("fury1l7hypmqk2yc334vc6vmdwzp5sdefygj2ad93p5")
	assert.NoError(t, err)
...
func TestKeeper_AccumulateDrops(t *testing.T) {
	...
	for _, in := range inputList {
		...
		assert.NoError(t, err)
		err = furyapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, address, in.Coins)
		assert.NoError(t, err)
	}
}
...
func TestKeeper_DistributeDrops_Fail(t *testing.T) {
	...
	err = furyapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, dispensationCreator, totalCoins)
	assert.NoError(t, err)
