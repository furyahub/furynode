...
import (
	...
	dispensationkeeper "github.com/Furynet/furynode/x/dispensation/keeper"
...
func TestNewLegacyQuerier(t *testing.T) {
	...
	keeper := furyapp.DispensationKeeper
	...
	qp, errRes := furyapp.LegacyAmino().MarshalJSON(&queryRecName)
	...
	err = furyapp.LegacyAmino().UnmarshalJSON(res, &dr)
	assert.Error(t, err)

}

func GenerateQueryData(app *app.FurynetApp, ctx sdk.Context, name string, outList []bank.Output) {
...
func TestQueryRecordsName(t *testing.T) {
	...
	GenerateQueryData(furyapp, ctx, name, outList)
	keeper := furyapp.DispensationKeeper
	querier := dispensationkeeper.NewLegacyQuerier(keeper)
	queryRecName := types.QueryRecordsByDistributionNameRequest{
		DistributionName: name,
...
func TestQueryRecordsAddr(t *testing.T) {
	...
	qp, errRes := furyapp.LegacyAmino().MarshalJSON(&quereyRecName)
	require.NoError(t, errRes)
	query.Path = ""
	query.Data = qp
