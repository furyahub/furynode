...
The CLI command 
```shell
furynoded tx dispensation run [Distribution_Name] [Distribution Type Airdrop/ValidatorSubsidy/LiquidityMining] --from [Authorised runner neeeds to sign the transaction] --yes --gas auto --gas-adjustment=1.5 --gas-prices 1.0fury --node tcp://rpc.furynet.finance:80 --chain-id furynet
```
sample CLI command
```shell
furynoded tx dispensation run 2_fury1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd LiquidityMining --from fury1l7hypmqk2yc334vc6vmdwzp5sdefygj2ad93p5 --yes --gas auto --gas-adjustment=1.5 --gas-prices 1.0fury --node tcp://rpc.furynet.finance:80 --chain-id furynet
```
sample output
```json
{
