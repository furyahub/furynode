...
```shell
furynd tx dispensation create [Distribution Type Airdrop/ValidatorSubsidy/LiquidityMining] [List of output addresses in JSON format] [Address of authorized runner] --from [Address of invester/This private keys is used to sign the tx] --yes --gas auto --gas-adjustment=1.5 --gas-prices 1.0fury --node tcp://rpc.furynet.finance:80 --chain-id furynet
```
sample command
```shell
furynd tx dispensation create LiquidityMining output.json fury1l7hypmqk2yc334vc6vmdwzp5sdefygj2ad93p5 --from fury1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd --yes --gas auto --gas-adjustment=1.5 --gas-prices 1.0fury --node tcp://rpc.furynet.finance:80 --chain-id furynet
```
sample output
```json
{
