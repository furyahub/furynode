...
import (
	...
	"github.com/Furynet/furynode/x/ethbridge/keeper"
...
func CreateTestApp(isCheckTx bool) (*app.FurynetApp, sdk.Context) {
	furynetApp := app.Setup(isCheckTx)
	ctx := furynetApp.BaseApp.NewContext(isCheckTx, tmproto.Header{})
	furynetApp.AccountKeeper.SetParams(ctx, authtypes.DefaultParams())
	initTokens := sdk.TokensFromConsensusPower(1000, sdk.DefaultPowerReduction)
	_ = app.AddTestAddrs(furynetApp, ctx, 6, initTokens)
	return furynetApp, ctx
}
...
func CreateSimulatorApp(isCheckTx bool) (sdk.Context, *app.FurynetApp) {
	furynetApp := app.Setup(isCheckTx)
	ctx := furynetApp.BaseApp.NewContext(isCheckTx, tmproto.Header{})
	furynetApp.TokenRegistryKeeper.SetRegistry(ctx, tokenregistryTypes.Registry{
	...
	return ctx, furynetApp
}

func CreateTestAppEthBridge(isCheckTx bool) (sdk.Context, keeper.Keeper) {
	furyapp, ctx := CreateTestApp(isCheckTx)
	return ctx, furyapp.EthbridgeKeeper
}
