...
import (
	...
	"github.com/Furynet/furynode/x/ethbridge/test"
...
func TestBurnEthSuccess(t *testing.T) {
	...
	for _, event := range res.Events {
		for _, attribute := range event.Attributes {
			...
			case "receiver":
				// multiple recipients, skip the comparison
				// require.Equal(t, value, types.TestBridgeModuleFury)
				_, err = sdk.AccAddressFromBech32(value)
				require.NoError(t, err)
			case "burner":
				// multiple recipients, skip the comparison
...
func TestUpdateCethReceiverAccountMsg(t *testing.T) {
	...
	err = furyapp.AddCoinsToAccount(types.ModuleName, bankKeeper, ctx, cosmosSender, coins)
	require.NoError(t, err)
