...
import (
	...
	clptest "github.com/Furynet/furynode/x/clp/test"
	clptypes "github.com/Furynet/furynode/x/clp/types"
	"github.com/Furynet/furynode/x/margin/keeper"
	"github.com/Furynet/furynode/x/margin/test"
...
func TestKeeper_Open(t *testing.T) {
	table := []struct {
		...
		},
		{
			...
			msgOpen: types.MsgOpen{
				Signer:           "fury1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
				CollateralAsset:  "xxx",
				CollateralAmount: sdk.NewUint(1000),
				BorrowAsset:      "xxx",
				Position:         types.Position_LONG,
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				err = furyapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, _signer, sdk.NewCoins(nativeCoin, externalCoin))
				require.Nil(t, err)
			} else {
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				_signer := clptest.GenerateAddress(clptest.AddressKey1)
				address = _signer.String()
				err = furyapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, _signer, sdk.NewCoins(nativeCoin))
				require.NoError(t, err)
...
func TestKeeper_ForceClose(t *testing.T) {
	table := []struct {
	...
	}{
		{
			...
			msgForceClose: types.MsgForceClose{
				Signer:     "fury1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
				MtpAddress: "fury1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
			},
			msgOpen: types.MsgOpen{
				CollateralAsset: "xxx",
				BorrowAsset:     "xxx",
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx, app := test.CreateTestAppMarginFromGenesis(false, func(app *furyapp.FurynetApp, genesisState furyapp.GenesisState) furyapp.GenesisState {
				...
				gs3 := &types.GenesisState{
					Params: &types.Params{
						...
						ForceCloseFundAddress:                    "fury1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
						IncrementalInterestPaymentFundPercentage: sdk.NewDecWithPrec(1, 1),
						IncrementalInterestPaymentFundAddress:    "fury1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
...
func TestKeeper_OpenClose(t *testing.T) {
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err = furyapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(nativeCoin, externalCoin))
			require.Nil(t, err)
...
func TestKeeper_OpenThenClose(t *testing.T) {
	...
	ctx, app := test.CreateTestAppMarginFromGenesis(false, func(app *furyapp.FurynetApp, genesisState furyapp.GenesisState) furyapp.GenesisState {
		...
		balances := []banktypes.Balance{
			{
				Address: "fury1pjm228rsgwqf23arkx7lm9ypkyma7mzr3y2n85",
				Coins: sdk.Coins{
