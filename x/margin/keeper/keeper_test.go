...
func TestKeeper_Borrow(t *testing.T) {
	...
	borrowTests := []struct {
		...
		},
		{
			...
			decimals:         18,
			to:               "fury",
			address:          "fury1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
			collateralAmount: sdk.NewUint(10000),
			custodyAmount:    sdk.NewUint(1000),
			leverage:         sdk.NewDec(1),
			health:           sdk.NewDec(1),
	...
	for _, tt := range borrowTests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				err := furyapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(nativeCoin))
				require.Nil(t, err)
			} else {
				address = tt.address
			}
...
func TestWhitelist(t *testing.T) {
	...
	marginKeeper.WhitelistAddress(ctx, "fury123")
	is := marginKeeper.IsWhitelisted(ctx, "fury123")
	require.True(t, is)
	whitelist, _, err := marginKeeper.GetWhitelist(ctx, nil)
	require.NoError(t, err)
	require.Equal(t, []string{"fury123"}, whitelist)
}
...
func initKeeper(t testing.TB) (sdk.Context, *furyapp.FurynetApp, types.Keeper) {
	ctx, app := test.CreateTestAppMargin(false)
	marginKeeper := app.MarginKeeper
	require.NotNil(t, marginKeeper)
	return ctx, app, marginKeeper
}
func addMTPKey(t testing.TB, ctx sdk.Context, app *furyapp.FurynetApp, marginKeeper types.Keeper, collateralAsset string, custodyAsset string, address string, position types.Position, id uint64, health sdk.Dec) types.MTP {
